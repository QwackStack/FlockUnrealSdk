...
enum class EProtokitePlaytestSetupSeverity : uint8
{
...
enum class EProtokitePlaytestSetupFix : uint8
{
...
struct PROTOKITEPLAYTESTEDITOR_API FProtokitePlaytestSetupFinding
{
	/** Stable, so tests and anything that filters findings can name one. */
	FName Id;
	EProtokitePlaytestSetupSeverity Severity = EProtokitePlaytestSetupSeverity::Info;
	FText Title;
	FText Detail;
	EProtokitePlaytestSetupFix Fix = EProtokitePlaytestSetupFix::OpenPlaytestSettings;
};

/** The settings a playtest depends on, read once so the findings can be decided without the editor. */
struct PROTOKITEPLAYTESTEDITOR_API FProtokitePlaytestSetupInput
{
	...
	static FProtokitePlaytestSetupInput FromProjectSettings();
};
 ...
 */
class PROTOKITEPLAYTESTEDITOR_API FProtokitePlaytestSetupStatus
{
public:
	...
	static TArray<FProtokitePlaytestSetupFinding> Evaluate(const FProtokitePlaytestSetupInput& Input);
};
