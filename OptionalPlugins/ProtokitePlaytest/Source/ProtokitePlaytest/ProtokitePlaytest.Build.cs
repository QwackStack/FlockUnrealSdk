...
public class ProtokitePlaytest : ModuleRules
{
	public ProtokitePlaytest(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		PrivateDefinitions.Add("WITH_PROTOKITE_PLAYTEST_VIDEO=" + (bBuildWithVideo ? "1" : "0"));
	...
	private void WarnAboutAPlaytestInAShippingBuild(ReadOnlyTargetRules Target)
	{
		...
		if (GameConfig.GetBool("/Script/ProtokitePlaytest.ProtokitePlaytestSettings", "bPlaytestingEnabled", out bPlaytestingEnabled) && bPlaytestingEnabled)
		{
			Logger.LogWarning("Protokite Playtest: Enable Playtesting is on in a Shipping build of {0}. Playtest builds should be Development builds: Shipping compiles logging out, so the Flock SDK cannot report error lines or failed ensures from it. Build a Development game for a playtest, or turn Enable Playtesting off in Project Settings > Plugins > Protokite Playtest Settings for a release.",
				Target.Name);
		}
		...
		{
			Logger.LogWarning("Protokite Playtest: this Shipping build of {0} carries a playtest's Game Version ({1}), so everything its players do is filed under that playtest. Point Game Version in Project Settings > Plugins > Flock SDK Settings at a release version before shipping it.",
