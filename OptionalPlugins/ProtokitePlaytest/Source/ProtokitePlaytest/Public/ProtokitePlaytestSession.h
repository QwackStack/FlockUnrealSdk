...
USTRUCT()
struct PROTOKITEPLAYTEST_API FProtokitePlaytestSessionStartResult
{
	GENERATED_BODY()
	 ...
	 */
	static bool FromWireObject(const TSharedRef<FJsonObject>& Object, FProtokitePlaytestSessionStartResult& OutResult, FString& OutError);
};

/** Protokite's answer to a session end, which carries nothing: it answers 204 with no body. */
USTRUCT()
struct PROTOKITEPLAYTEST_API FProtokitePlaytestSessionEndResult
{
	GENERATED_BODY()

	/** Accepts any JSON object, since the answer carries nothing to read. */
	static bool FromWireObject(const TSharedRef<FJsonObject>& Object, FProtokitePlaytestSessionEndResult& OutResult, FString& OutError);
};

/** Protokite's answer to a feedback form, which carries the stored response; nothing here needs reading back. */
USTRUCT()
struct PROTOKITEPLAYTEST_API FProtokitePlaytestFormSubmitResult
{
	GENERATED_BODY()

	/** Accepts any JSON object: that the server took it is the whole answer. */
	static bool FromWireObject(const TSharedRef<FJsonObject>& Object, FProtokitePlaytestFormSubmitResult& OutResult, FString& OutError);
};

/** Everything a session start sends apart from the request headers. */
struct PROTOKITEPLAYTEST_API FProtokitePlaytestSessionStartRequest
{
 ...
 */
PROTOKITEPLAYTEST_API TMap<FString, FString> MakePlaytestSessionDebugInfo(const FString& MapName,
	EProtokitePlaytestConsentChoice PlayerConsent, bool bAskedThePlayer);
