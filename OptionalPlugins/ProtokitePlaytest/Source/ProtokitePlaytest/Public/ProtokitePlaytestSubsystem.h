...
#include "ProtokitePlaytestSubsystem.generated.h"

class FProtokitePlaytestRecordingRun;
class FProtokitePlaytestVideoRecording;
class FProtokitePlaytestFormKeyWatcher;
class FProtokitePlaytestRecordingUploads;
class SProtokitePlaytestConsentWidget;
class SProtokitePlaytestFormWidget;
class FProtokiteClient;
class IFlockFileUploader;
class IProtokitePlaytestVideoFrameSource;
class UFlockSubsystem;
enum class EProtokitePlaytestVideoStopReason : uint8;
class UGameInstance;
class UWorld;
struct FWorldContext;

/** Raised when this launch's recording finishes: uploaded, or not, with the reason it did not go. */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FProtokitePlaytestRecordingUploadFinished, bool, bUploaded, const FString&, WhyNot);
 ...
 * VP9 file under Saved/ProtokitePlaytest/Recordings/Playtest, with the Video Recording settings. One recording is made per
 ...
 * can be tried without a playtest, saved under Recordings/TestVideos: Record Video In Play In Editor (Protokite Playtest Local
 * Settings), or the console command ProtokitePlaytest.RecordTestVideo <seconds> in builds that are not Shipping.
 *
 ...
 */
UCLASS()
class PROTOKITEPLAYTEST_API UProtokitePlaytestSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
...
public:
	...
	EProtokitePlaytestStatus GetStatus() const { return Status; }

	/** This build's playtest config. Empty unless it has been fetched for the current Flock initialization. */
	const FProtokitePlaytestConfig& GetPlaytestConfig() const { return PlaytestConfig; }

	/**
	 * True only while GetStatus() is Ready, the player's consent allows FeatureName, and the playtest's config turns it
	 * on (see ProtokitePlaytestFeatures). A feature the config does not mention is off.
	 *
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = "Protokite|Playtest")
	EProtokitePlaytestConsentChoice GetPlaytestConsent() const;

	/** What the player answered, and NotAnswered when they have not. Unlike GetPlaytestConsent, it never stands in for them. */
	UFUNCTION(BlueprintPure, Category = "Protokite|Playtest")
	EProtokitePlaytestConsentChoice GetPlayersConsentAnswer() const;
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Protokite|Playtest")
	bool SetPlaytestConsent(EProtokitePlaytestConsentChoice Choice);
	...
	EProtokitePlaytestSessionState GetPlaytestSessionState() const { return SessionState; }
	...
	const FProtokitePlaytestIdentity& GetPlaytestIdentity() const { return PlaytestIdentity; }
	 ...
	 */
	UPROPERTY(BlueprintAssignable, Category = "Protokite|Playtest")
	FProtokitePlaytestRecordingUploadFinished OnRecordingUploadFinished;
	 ...
	 * (ProtokitePlaytestEvents), and when the Flock SDK refuses the event: its analytics off (Analytics Enabled), consent
	 ...
	 */
	bool SendFilledInForm(const FProtokitePlaytestFormAnswers& Answers);
	...
	void SetSteamAccountReaderForTesting(TFunction<FProtokitePlaytestRunningSteamAccount()> Reader) { TestSteamAccountReader = MoveTemp(Reader); }
	 ...
	 */
	void SetVideoFrameSourceFactoryForTesting(TFunction<TSharedPtr<IProtokitePlaytestVideoFrameSource>(FIntPoint MaxVideoSize, FString& OutWhyNot)> Factory)
	{
		TestVideoFrameSourceFactory = MoveTemp(Factory);
	}

	/**
	 * Keeps video recordings in Folder instead of Saved/ProtokitePlaytest/Recordings, and finishes what ended runs left there
...
private:
	...
	/** The folder recordings are kept in: Saved/ProtokitePlaytest/Recordings, or the testing folder. */
	...
	/** The file the player's answer is kept in: Saved/ProtokitePlaytest/playtest_consent.json, or the testing one. */
	...
	TSharedRef<FProtokitePlaytestRecordingUploads> GetOrCreateRecordingUploads();
	...
	void FinishVideoRecordingNow(EProtokitePlaytestVideoStopReason Reason);
	...
	EProtokitePlaytestStatus DecideCurrentStatus() const;

	/** The Protokite client, created on first use with the Flock SDK's timeout and retry settings. */
	TSharedRef<FProtokiteClient> GetOrCreateProtokiteClient();
	 ...
	 */
	FProtokitePlaytestIdentity ResolvePlaytestIdentity(FString& OutWhyNone) const;

	/** The only writer of Status. Logs when the status changes. */
	void ApplyStatus(EProtokitePlaytestStatus NewStatus, const FString& ProtokiteApiUrl, const FString& GameVersionId);

	TWeakObjectPtr<UFlockSubsystem> Flock;

	EProtokitePlaytestStatus Status = EProtokitePlaytestStatus::TurnedOff;
	...
	FProtokitePlaytestConfig PlaytestConfig;
	EProtokitePlaytestConfigState ConfigState = EProtokitePlaytestConfigState::NotFetched;
	...
	EProtokitePlaytestSessionState SessionState = EProtokitePlaytestSessionState::NotStarted;
	FString PlaytestSessionId;
	FProtokitePlaytestIdentity PlaytestIdentity;
	...
	FProtokitePlaytestPerformanceTimeline PerformanceTimeline;
	...
	TSharedPtr<FProtokiteClient> ProtokiteClient;
	TSharedPtr<IFlockHttpAdapter> TestHttpAdapter;
	TOptional<FFlockRetryPolicy> TestRetryPolicy;
	TFunction<FProtokitePlaytestRunningSteamAccount()> TestSteamAccountReader;
	...
	TSharedPtr<FProtokitePlaytestVideoRecording> VideoRecording;
	 ...
	 */
	TSharedPtr<FProtokitePlaytestRecordingRun> VideoRecordingRun;
	...
	TSharedPtr<SProtokitePlaytestFormWidget> FormWidget;

	/** Watches for the key that opens the form. Only alive while there is a form to open. */
	TSharedPtr<FProtokitePlaytestFormKeyWatcher> FormKeyWatcher;
	...
	TSharedPtr<SProtokitePlaytestConsentWidget> ConsentWidget;

	/** The player's saved answer, read from its file the first time it is asked for. */
	mutable TOptional<EProtokitePlaytestConsentChoice> SavedConsentChoice;
	...
	TSharedPtr<FProtokitePlaytestRecordingUploads> RecordingUploads;
	...
	TFunction<TSharedPtr<IProtokitePlaytestVideoFrameSource>(FIntPoint, FString&)> TestVideoFrameSourceFactory;
