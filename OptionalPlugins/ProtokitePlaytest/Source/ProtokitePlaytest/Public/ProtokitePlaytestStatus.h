...
#include "ProtokitePlaytestStatus.generated.h"

/** Whether this build may do playtest work right now, and if not, why. */
UENUM(BlueprintType)
enum class EProtokitePlaytestStatus : uint8
{
...
enum class EProtokitePlaytestConfigState : uint8
{
	NotFetched,
	Fetching,
	Loaded,
...
struct FProtokitePlaytestStatusInputs
{
	 ...
	 */
	EProtokitePlaytestConsentChoice PlayerConsent = EProtokitePlaytestConsentChoice::NotAnswered;
};
 ...
 */
PROTOKITEPLAYTEST_API EProtokitePlaytestStatus DecidePlaytestStatus(const FProtokitePlaytestStatusInputs& Inputs);
 ...
 * loaded.
 */
PROTOKITEPLAYTEST_API EProtokitePlaytestConfigState DecidePlaytestConfigState(const TFlockResult<FProtokitePlaytestConfig>& Result,
 ...
 */
PROTOKITEPLAYTEST_API bool IsUsableProtokiteApiUrl(const FString& Url);

/** One sentence describing the status, naming the setting to change when there is one. */
PROTOKITEPLAYTEST_API FString DescribePlaytestStatus(EProtokitePlaytestStatus Status);
