...
namespace ProtokitePlaytestFixtureTesting
{
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestFixtureKeepsItsFilesTest, "Protokite.Playtest.Fixture.KeepsItsFilesOutOfTheProject",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestFixtureKeepsItsFilesTest::RunTest(const FString& Parameters)
{
	...
	const FString ProjectLaunchesBefore = ProtokitePlaytestFixtureTesting::ProjectLaunchFolders();
	const FString ProjectFormsBefore = ProtokitePlaytestFixtureTesting::FilesUnder(FProtokitePlaytestFormSpool::GetDefaultFolder());
	...
	FString FixtureFolder;
	{
		...
		Fixture.Transport->Answer(TEXT("/game/sdk/feedback-form"), FProtokitePlaytestFakeTransport::Status(500, TEXT("{}")));
		Fixture.StartFlock();
		...
		TestEqualSensitive(TEXT("Not one launch folder is added to the project's, or taken from it"),
			ProtokitePlaytestFixtureTesting::ProjectLaunchFolders(), ProjectLaunchesBefore);
		...
		TestEqual(TEXT("A form that could not be sent is kept in the fixture's own folder"),
			FProtokitePlaytestFormSpool(FPaths::Combine(Fixture.Folder, TEXT("FeedbackForms"))).CountWaiting(), 1);
		TestEqualSensitive(TEXT("Not in the project's"),
			ProtokitePlaytestFixtureTesting::FilesUnder(FProtokitePlaytestFormSpool::GetDefaultFolder()), ProjectFormsBefore);
	}
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestFixtureIsTheOnlyWayTest, "Protokite.Playtest.Fixture.EveryTestBuildsTheSdkThroughIt",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestFixtureIsTheOnlyWayTest::RunTest(const FString& Parameters)
{
	...
	TestEqualSensitive(TEXT("Only FPlaytestFixture builds a Flock SDK"), FString::Join(FilesBuildingOne, TEXT(", ")),
		FString(TEXT("ProtokitePlaytestSubsystemTestSupport.h")));
	return true;
}
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestFixtureSettingsAreTheTestsOwnTest,
	"Protokite.Playtest.Fixture.EveryTestSaysWhatItsSettingsAre",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestFixtureSettingsAreTheTestsOwnTest::RunTest(const FString& Parameters)
{
	const FString TestsFolder = FPaths::ConvertRelativePathToFull(FPaths::GetPath(FString(ANSI_TO_TCHAR(__FILE__))));
