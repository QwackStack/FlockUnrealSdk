...
namespace
{
	...
	{
	public:
		FCheckRecordedGameViewportCommand(FAutomationTestBase* InTest, UProtokitePlaytestSubsystem* InPlaytest)
			: Test(InTest)
			, Playtest(InPlaytest)
		...
		{
			const UProtokitePlaytestSubsystem* Subsystem = Playtest.Get();
			...
			Test->TestTrue(FString::Printf(TEXT("Saved as a test video under the project's recordings folder (%s)"), *Path),
				FPaths::IsUnderDirectory(Path, FProtokitePlaytestRecordingsFolder::GetKindFolder(FProtokitePlaytestRecordingsFolder::GetDefaultPath(),
					EProtokitePlaytestRecordingKind::TestVideo)));
	...
	{
	public:
		FRecordThroughAResizeCommand(FAutomationTestBase* InTest, UWorld* InWorld, const TSharedPtr<FProtokitePlaytestVideoRecording>& InRecording,
	...
	private:
		...
		TSharedPtr<FProtokitePlaytestVideoRecording> Recording;
		FString Folder;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestVideoKeepsRecordingThroughAWindowResizeTest,
	"Protokite.Playtest.Video.KeepsRecordingThroughAWindowResize",
	EAutomationTestFlags::ClientContext | EAutomationTestFlags::NonNullRHI | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestVideoKeepsRecordingThroughAWindowResizeTest::RunTest(const FString& Parameters)
{
	...
	FString WhyNot;
	const TSharedPtr<IProtokitePlaytestVideoFrameSource> Source = FProtokitePlaytestGameViewportFrameSource::Create(GameInstance, FIntPoint(1280, 720), WhyNot);
	...
	FString Error;
	const TSharedPtr<FProtokitePlaytestVideoRecording> Recording = FProtokitePlaytestVideoRecording::Start(Source.ToSharedRef(), FProtokitePlaytestVideoSettings(),
...
// -nullrhi run.
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestVideoRecordsTheGameViewportTest,
	"Protokite.Playtest.Video.RecordsTheGameViewport",
	EAutomationTestFlags::ClientContext | EAutomationTestFlags::NonNullRHI | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestVideoRecordsTheGameViewportTest::RunTest(const FString& Parameters)
{
	UWorld* World = GEngine != nullptr && GEngine->GameViewport != nullptr ? GEngine->GameViewport->GetWorld() : nullptr;
	UGameInstance* GameInstance = World != nullptr ? World->GetGameInstance() : nullptr;
	...
	TestTrue(TEXT("The console command is known"), GEngine->Exec(World, TEXT("ProtokitePlaytest.RecordTestVideo 2")));
