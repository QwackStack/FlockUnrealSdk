...
namespace ProtokitePlaytestVideoTesting
{
	...
	inline TArray<FString> RecordingFilesIn(const FString& Folder)
	{
		...
		Found = Found.FilterByPredicate([](const FString& Path)
		{
			...
			return !Name.Equals(FProtokitePlaytestRecordingsFolder::LockFileName, ESearchCase::IgnoreCase)
				&& !Name.Equals(FProtokitePlaytestRecordingsFolder::SessionFileName, ESearchCase::IgnoreCase)
				&& !Name.Equals(FProtokitePlaytestRecordingsFolder::ReservedBytesFileName, ESearchCase::IgnoreCase);
		});
		Found.Sort();
		return Found;
	}
	 ...
	 */
	class FTestVideoFrameSource : public IProtokitePlaytestVideoFrameSource
	{
	public:
		...
		{
			++FramesAskedFor;
			FProtokitePlaytestVideoFrame& Frame = Arrived.AddDefaulted_GetRef();
