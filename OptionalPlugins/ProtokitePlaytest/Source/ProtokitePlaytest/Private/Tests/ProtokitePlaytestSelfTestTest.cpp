...
namespace
{
	using EOutcome = EProtokitePlaytestSelfTestOutcome;
	...
	{
		return FString(TEXT("/game/sdk/playtest-session/")) + FProtokitePlaytestSelfTest::SessionIdThatDoesNotExist + Tail;
	}
	...
	const FProtokitePlaytestSelfTestStep* FindSelfTestStep(const TArray<FProtokitePlaytestSelfTestStep>& Steps, const TCHAR* Name)
	{
		return Steps.FindByPredicate([Name](const FProtokitePlaytestSelfTestStep& Step) { return Step.Name.Equals(Name, ESearchCase::CaseSensitive); });
	}

	/** Checks one step's outcome, and that its detail says DetailMustContain when that is given. */
	void ExpectSelfTestStep(FAutomationTestBase& Test, const TArray<FProtokitePlaytestSelfTestStep>& Steps, const TCHAR* Name,
		EOutcome Outcome, const FString& DetailMustContain = FString())
	{
		const FProtokitePlaytestSelfTestStep* Step = FindSelfTestStep(Steps, Name);
	...
	bool TickUntilTheSelfTestFinishes(const TSharedRef<FProtokitePlaytestSelfTest>& Run, float Seconds)
	{
	...
	FProtokitePlaytestSelfTest::FOptions SelfTestOptions(const FPlaytestFixture& Fixture, const FString& ClosedGameVersionId = FString())
	{
	...
	void AnswerEveryCounterCase(FPlaytestFixture& Fixture)
	{
		...
		Fixture.Transport->AnswerInOrder(PlaytestConfigRoute, {
			FProtokitePlaytestFakeTransport::Status(401, InvalidApiKeyBody),
			FProtokitePlaytestFakeTransport::Status(422, MissingApiKeyBody),
			FProtokitePlaytestFakeTransport::Status(404, NotLinkedBody) });
		Fixture.Transport->AnswerInOrder(PlaytestSessionStartRoute, {
			FProtokitePlaytestFakeTransport::Status(422, NoPlayerIdentityBody),
			FProtokitePlaytestFakeTransport::Status(400, NoLongerCollectingBody) });
		Fixture.Transport->AnswerInOrder(SelfTestFeedbackFormRoute, {
			FProtokitePlaytestFakeTransport::Status(422, SelfTestComplaintBody(TEXT("Missing required answer 'rating'"))),
			FProtokitePlaytestFakeTransport::Status(422, SelfTestComplaintBody(TEXT("Invalid option for 'category'"))),
			FProtokitePlaytestFakeTransport::Status(404, SelfTestSessionNotFoundBody),
			FProtokitePlaytestFakeTransport::Status(200, Envelope(TEXT("{\"id\":\"01KX0FORMRESPONSE000000001\"}"))) });
	...
	TArray<FFlockHttpRequest> SelfTestRequestsTo(const FProtokitePlaytestFakeTransport& Transport, const FString& Route, int32 FirstIndex)
	{
	...
	TSharedRef<FProtokitePlaytestFakeFileUploader> RecordForTheSelfTest(FPlaytestFixture& Fixture)
	{
		...
		Fixture.Transport->Answer(SelfTestLaunchUploadLinkRoute(), FProtokitePlaytestFakeTransport::Status(200, Envelope(
			TEXT("{\"upload_url\":\"http://storage.local/put/self-test.webm?signature=abc\",\"bucket\":\"b\",\"key\":\"k\"}"))));
		Fixture.Playtest->SetVideoFrameSourceFactoryForTesting([](FIntPoint MaxVideoSize, FString&) -> TSharedPtr<IProtokitePlaytestVideoFrameSource>
		{
			return MakeShared<ProtokitePlaytestVideoTesting::FTestVideoFrameSource>(
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestJudgeAcceptedIsNotARefusalTest,
	"Protokite.Playtest.SelfTest.Judge.AnAcceptedRequestIsNotTheRefusal",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestJudgeAcceptedIsNotARefusalTest::RunTest(const FString& Parameters)
{
	FString WhatHappened;
	TestFalse(TEXT("A success fails the step"),
		FProtokitePlaytestSelfTest::IsTheExpectedRefusal(true, FFlockError(), 404, FString(), WhatHappened));
	TestTrue(TEXT("Saying it was accepted"), WhatHappened.Contains(TEXT("accepted")));

	TestTrue(TEXT("The refusal it expects passes"), FProtokitePlaytestSelfTest::IsTheExpectedRefusal(false,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestJudgeOtherStatusTest,
	"Protokite.Playtest.SelfTest.Judge.AnotherStatusIsNotTheRefusal",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestJudgeOtherStatusTest::RunTest(const FString& Parameters)
{
	FString WhatHappened;
	TestFalse(TEXT("A 500 is not the 404 expected"), FProtokitePlaytestSelfTest::IsTheExpectedRefusal(false,
	...
	TestFalse(TEXT("A 422 is not the 401 a wrong key gets"), FProtokitePlaytestSelfTest::IsTheExpectedRefusal(false,
	...
	TestFalse(TEXT("A request that never reached Protokite is no refusal at all"),
		FProtokitePlaytestSelfTest::IsTheExpectedRefusal(false, NeverReached, 404, FString(), WhatHappened));
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestJudgeNamedQuestionTest,
	"Protokite.Playtest.SelfTest.Judge.ARefusalMustNameTheQuestion",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestJudgeNamedQuestionTest::RunTest(const FString& Parameters)
{
	FString WhatHappened;
	TestTrue(TEXT("Naming the question passes"), FProtokitePlaytestSelfTest::IsTheExpectedRefusal(false,
		SelfTestErrorWithStatus(422, TEXT("Missing required answer 'rating'")), 422, TEXT("rating"), WhatHappened));

	TestFalse(TEXT("Naming another question fails"), FProtokitePlaytestSelfTest::IsTheExpectedRefusal(false,
	...
	TestFalse(TEXT("The same question in other letters fails: the server names its questions letter for letter"),
		FProtokitePlaytestSelfTest::IsTheExpectedRefusal(false, SelfTestErrorWithStatus(422, TEXT("Missing required answer 'Rating'")),
			422, TEXT("rating"), WhatHappened));

	TestFalse(TEXT("Naming no question fails"), FProtokitePlaytestSelfTest::IsTheExpectedRefusal(false,
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestOptionNotOnTheListTest,
	"Protokite.Playtest.SelfTest.MakesAnOptionThatIsNotOnTheList",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestOptionNotOnTheListTest::RunTest(const FString& Parameters)
{
	...
	{
		const FString Made = FProtokitePlaytestSelfTest::MakeOptionNotOnTheList(Options);
	...
	TestEqualSensitive(TEXT("Where it can, it is the first option in other letters"),
		FProtokitePlaytestSelfTest::MakeOptionNotOnTheList({ TEXT("Bug"), TEXT("Crash") }), TEXT("BUG"));
	return true;
}
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestFullRunTest,
	"Protokite.Playtest.SelfTest.EveryStepPassesAgainstAPlaytestThatTurnsEverythingOn",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestFullRunTest::RunTest(const FString& Parameters)
{
	...
	const TSharedRef<FProtokitePlaytestFakeFileUploader> Uploader = RecordForTheSelfTest(Fixture);
	...
	const bool bBuiltWithVideo = FProtokitePlaytestVideoEncoder::IsBuiltWithVideo();
	if (!TestEqual(TEXT("Precondition: the launch's session started"), static_cast<int32>(Fixture.Playtest->GetPlaytestSessionState()), static_cast<int32>(EProtokitePlaytestSessionState::Started))
	...
	const TSharedRef<FProtokitePlaytestSelfTest> Run = FProtokitePlaytestSelfTest::Start(Fixture.Playtest, Fixture.Flock,
	...
	const TArray<FProtokitePlaytestSelfTestStep>& Steps = Run->GetSteps();
	...
	if (TestEqual(TEXT("Three config requests were sent"), ConfigProbes.Num(), 3))
	{
		TestEqualSensitive(TEXT("The first with a wrong key"), ConfigProbes[0].Headers.FindRef(TEXT("X-Flock-API-Key")),
			FProtokitePlaytestSelfTest::WrongApiKey);
		...
		TestEqualSensitive(TEXT("And a version no playtest is linked to"), ConfigProbes[2].Headers.FindRef(TEXT("X-Game-Version-ID")),
			FProtokitePlaytestSelfTest::GameVersionIdWithNoPlaytest);
	}
	...
	if (TestEqual(TEXT("Four forms were sent"), Forms.Num(), 4))
	{
		...
		TestEqualSensitive(TEXT("The third names a session that does not exist"), StringMember(SelfTestParseBody(Forms[2]), TEXT("session_id")),
			FProtokitePlaytestSelfTest::SessionIdThatDoesNotExist);
	...
	TestEqual(TEXT("And the playtest shows it ended"), static_cast<int32>(Fixture.Playtest->GetPlaytestSessionState()), static_cast<int32>(EProtokitePlaytestSessionState::Ended));
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestAcceptedRefusalsFailTest,
	"Protokite.Playtest.SelfTest.AnAcceptedCounterCaseFailsAndItsSessionIsEnded",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestAcceptedRefusalsFailTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	...
	Fixture.AnswerConfig(FProtokitePlaytestFakeTransport::Status(200, ConfigBody()));
	Fixture.Transport->Answer(PlaytestSessionStartRoute,
		FProtokitePlaytestFakeTransport::Status(200, SessionStartBody(SelfTestWronglyCreatedSessionId)));
	Fixture.Transport->Answer(SelfTestFeedbackFormRoute,
		FProtokitePlaytestFakeTransport::Status(200, Envelope(TEXT("{\"id\":\"01KX0FORMRESPONSE000000001\"}"))));
	Fixture.Transport->Answer(SelfTestUnknownSessionRoute(TEXT("/recording-upload")), FProtokitePlaytestFakeTransport::Status(200, Envelope(
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestSkipsWithoutConsentTest,
	"Protokite.Playtest.SelfTest.SkipsWhenNobodyHasSaidWhatItMayCollect",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestSkipsWithoutConsentTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedFlockAnalyticsSettings FlockAnalytics(true);
	...
	const TSharedRef<FProtokitePlaytestSelfTest> Run = FProtokitePlaytestSelfTest::Start(Fixture.Playtest, Fixture.Flock, SelfTestOptions(Fixture));
	TestTrue(TEXT("The run finishes"), TickUntilTheSelfTestFinishes(Run, 10.f));

	using namespace ProtokitePlaytestSelfTestSteps;
	const TArray<FProtokitePlaytestSelfTestStep>& Steps = Run->GetSteps();
	// Not a failure, and it names the command a run with nobody there answers with.
	ExpectSelfTestStep(*this, Steps, PlaytestLoaded, EOutcome::Skipped, TEXT("ProtokitePlaytest.AnswerConsent"));
	...
	const TSharedRef<FProtokitePlaytestSelfTest> AnsweredRun =
		FProtokitePlaytestSelfTest::Start(Answered.Playtest, Answered.Flock, SelfTestOptions(Answered));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestSkipsWhatIsOffTest,
	"Protokite.Playtest.SelfTest.SkipsWhatThePlaytestDoesNotTurnOn",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestSkipsWhatIsOffTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedFlockAnalyticsSettings FlockAnalytics(true);
	FPlaytestFixture Fixture;
	Fixture.AnswerConfig(FProtokitePlaytestFakeTransport::Status(200, Envelope(FString::Printf(
	...
	using namespace ProtokitePlaytestSelfTestSteps;
	const TArray<FProtokitePlaytestSelfTestStep>& Steps = Run->GetSteps();
	ExpectSelfTestStep(*this, Steps, ClosedPlaytestRefused, EOutcome::Skipped, TEXT("ProtokitePlaytest.SelfTest <Game Version ID>"));
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestNeedsASignInTest,
	"Protokite.Playtest.SelfTest.WithNobodySignedInTheSessionStepFailsAndWhatNeedsItIsSkipped",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestNeedsASignInTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedFlockAnalyticsSettings FlockAnalytics(true);
	FPlaytestFixture Fixture;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestNeedsThePlaytestTest,
	"Protokite.Playtest.SelfTest.WhenThePlaytestDoesNotLoadNothingElseRuns",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestNeedsThePlaytestTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedFlockAnalyticsSettings FlockAnalytics(true);
	FPlaytestFixture Fixture;
	Fixture.AnswerConfig(FProtokitePlaytestFakeTransport::Status(404, NotLinkedBody));
	Fixture.StartFlock();
	const int32 RequestsBefore = Fixture.Transport->Requests.Num();
	...
	const TArray<FProtokitePlaytestSelfTestStep>& Steps = Run->GetSteps();
	ExpectSelfTestStep(*this, Steps, ProtokitePlaytestSelfTestSteps::PlaytestLoaded, EOutcome::Failed, TEXT("PlaytestNotLinked"));
	TestEqual(TEXT("Every other step was skipped"), Steps.FilterByPredicate([](const FProtokitePlaytestSelfTestStep& Step)
		{
 ...
 * refused.
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestReportsNotWatchableTest,
	"Protokite.Playtest.SelfTest.SaysWhenReportsCannotBeWatched",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestReportsNotWatchableTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestSendsEachCounterCaseOnceTest,
	"Protokite.Playtest.SelfTest.SendsEachCounterCaseOnce",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestSendsEachCounterCaseOnceTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedFlockAnalyticsSettings FlockAnalytics(true);
	...
	const FFlockHttpResponse Failing = FProtokitePlaytestFakeTransport::Status(500, TEXT("{\"detail\":\"Internal Server Error\"}"));
	...
	ExpectSelfTestStep(*this, Run->GetSteps(), ProtokitePlaytestSelfTestSteps::WrongApiKeyRefused, EOutcome::Failed, TEXT("HTTP 500"));
	return true;
}
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestRepeatQueuedFailsTest,
	"Protokite.Playtest.SelfTest.AnExceptionWhoseRepeatIsQueuedFailsTheStep",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestRepeatQueuedFailsTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedFlockAnalyticsSettings FlockAnalytics(true, /*bCaptureExceptions*/ true, /*bKeepEventsUntilSent*/ true);
	...
	ExpectSelfTestStep(*this, Run->GetSteps(), ProtokitePlaytestSelfTestSteps::ExceptionReported, EOutcome::Failed, TEXT("queued 2 reports"));
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestEventNotSentFailsTest,
	"Protokite.Playtest.SelfTest.AnEventStillQueuedAfterTheFlushFailsTheStep",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestEventNotSentFailsTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedFlockAnalyticsSettings FlockAnalytics(true, /*bCaptureExceptions*/ true, /*bKeepEventsUntilSent*/ true);
	FPlaytestFixture Fixture;
	...
	const TSharedRef<FProtokitePlaytestSelfTest> Run = FProtokitePlaytestSelfTest::Start(Fixture.Playtest, Fixture.Flock, SelfTestOptions(Fixture));
	TestTrue(TEXT("The run finishes"), TickUntilTheSelfTestFinishes(Run, 10.f));
	ExpectSelfTestStep(*this, Run->GetSteps(), ProtokitePlaytestSelfTestSteps::PlaytestEventRecorded, EOutcome::Failed, TEXT("still queued"));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestSessionAlreadyEndedTest,
	"Protokite.Playtest.SelfTest.ASessionThatHasAlreadyEndedFailsTheSessionStep",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestSessionAlreadyEndedTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedFlockAnalyticsSettings FlockAnalytics(true);
	FPlaytestFixture Fixture;
...
#if WITH_PROTOKITE_PLAYTEST_VIDEO
...
{
	TStrongObjectPtr<UProtokitePlaytestSelfTestListener> Listener{ NewObject<UProtokitePlaytestSelfTestListener>() };
	TSharedRef<TArray<TPair<bool, FString>>> Heard = MakeShared<TArray<TPair<bool, FString>>>();

	explicit FSelfTestUploadHeard(UProtokitePlaytestSubsystem* Playtest)
	{
		...
		Playtest->OnRecordingUploadFinished.AddDynamic(Listener.Get(), &UProtokitePlaytestSelfTestListener::HandleRecordingUploadFinished);
	}
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestUploadFinishedWhenUploadedTest,
	"Protokite.Playtest.RecordingUpload.RaisesUploadFinishedOnceUploaded",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestUploadFinishedWhenUploadedTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FPlaytestFixture Fixture;
	const TSharedRef<FProtokitePlaytestFakeFileUploader> Uploader = RecordForTheSelfTest(Fixture);
	Fixture.AnswerConfig(FProtokitePlaytestFakeTransport::Status(200, ConfigBody(GameVersionId, false, /*bVideoRecording*/ true)));
	Fixture.StartFlock();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestUploadFinishedWhenFailedTest,
	"Protokite.Playtest.RecordingUpload.RaisesUploadFinishedWithTheReasonWhenItFails",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestUploadFinishedWhenFailedTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FPlaytestFixture Fixture;
	const TSharedRef<FProtokitePlaytestFakeFileUploader> Uploader = RecordForTheSelfTest(Fixture);
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestUploadFinishedWhenItCannotBeginTest,
	"Protokite.Playtest.RecordingUpload.RaisesUploadFinishedWhenItCannotBegin",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestUploadFinishedWhenItCannotBeginTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FPlaytestFixture Fixture;
	const TSharedRef<FProtokitePlaytestFakeFileUploader> Uploader = RecordForTheSelfTest(Fixture);
	...
	RecordHalfASecond(Fixture);
	FSelfTestUploadHeard Heard(Fixture.Playtest);
	TestEqual(TEXT("Precondition: no session started"), static_cast<int32>(Fixture.Playtest->GetPlaytestSessionState()), static_cast<int32>(EProtokitePlaytestSessionState::NotStarted));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestUploadFinishedNotForTestVideoTest,
	"Protokite.Playtest.RecordingUpload.RaisesNothingForATestVideo",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestUploadFinishedNotForTestVideoTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FPlaytestFixture Fixture;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestUploadFinishedAfterAStopTest,
	"Protokite.Playtest.RecordingUpload.AStopForGoodUploadsItToo",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestUploadFinishedAfterAStopTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FPlaytestFixture Fixture;
	const TSharedRef<FProtokitePlaytestFakeFileUploader> Uploader = RecordForTheSelfTest(Fixture);
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestUploadFinishedNotWhileClosingTest,
	"Protokite.Playtest.RecordingUpload.RaisesNothingWhileTheGameCloses",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestUploadFinishedNotWhileClosingTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FPlaytestFixture Fixture;
	const TSharedRef<FProtokitePlaytestFakeFileUploader> Uploader = RecordForTheSelfTest(Fixture);
 ...
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestUploadFinishedAfterTheTickerSettlesTest,
	"Protokite.Playtest.RecordingUpload.RaisesItOnceTheRecordingIsPutAway",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestUploadFinishedAfterTheTickerSettlesTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FPlaytestFixture Fixture;
	RecordForTheSelfTest(Fixture);
	...
	RecordHalfASecond(Fixture);
	FSelfTestUploadHeard Heard(Fixture.Playtest);
	UProtokitePlaytestSubsystem* Playtest = Fixture.Playtest;
...
#endif // WITH_PROTOKITE_PLAYTEST_VIDEO

/** A game that goes away mid-run is one failure, not one for every step left: the rest are skipped, saying why. */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestSelfTestGameShutDownTest,
	"Protokite.Playtest.SelfTest.AGameThatShutsDownMidRunFailsOnce",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestSelfTestGameShutDownTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedFlockAnalyticsSettings FlockAnalytics(true);
	FPlaytestFixture Fixture;
	...
	const TArray<FProtokitePlaytestSelfTestStep>& Steps = Run->GetSteps();
	TestEqual(TEXT("One step fails for it"), Steps.FilterByPredicate([](const FProtokitePlaytestSelfTestStep& Step)
		{
			return Step.Outcome == EOutcome::Failed;
		}).Num(), 1);
	ExpectSelfTestStep(*this, Steps, ProtokitePlaytestSelfTestSteps::SessionStarted, EOutcome::Failed, TEXT("the game shut down"));
	ExpectSelfTestStep(*this, Steps, ProtokitePlaytestSelfTestSteps::SessionEnded, EOutcome::Skipped, TEXT("the game shut down"));
	return true;
}
