...
namespace ProtokitePlaytestSubsystemTesting
{
	...
	{
		UProtokitePlaytestSettings* Settings;
		...
		FScopedPlaytestSettings(bool bPlaytestingEnabled, const FString& ProtokiteApiUrl,
			...
			: Settings(GetMutableDefault<UProtokitePlaytestSettings>())
	...
	struct FScopedTestWorld
	{
		...
		{
			UPackage* Package = CreatePackage(*FString::Printf(TEXT("/Temp/ProtokitePlaytestTests/%s/%s"),
	...
	inline FFlockInitConfig MakeFlockConfig(const FString& GameVersionId = ProtokitePlaytestFixtures::GameVersionId)
	{
	...
	struct FPlaytestFixture
	{
		...
		UProtokitePlaytestSubsystem* Playtest = NewObject<UProtokitePlaytestSubsystem>(GameInstance);
		...
		TSharedRef<FProtokitePlaytestRunningSteamAccount> SteamAccount = MakeShared<FProtokitePlaytestRunningSteamAccount>();
		...
		explicit FPlaytestFixture(bool bTurnRetriesOff = true, bool bUseTestIdentitySources = true,
			EProtokitePlaytestConsentChoice PlayersConsent = EProtokitePlaytestConsentChoice::VideoAndPlayData)
		{
			...
			FProtokitePlaytestConsentFile(ConsentFilePath).Save(PlayersConsent);

			AnswerConfig(FProtokitePlaytestFakeTransport::Status(200, ProtokitePlaytestFixtures::ConfigBody()));
			Transport->Answer(ProtokitePlaytestFixtures::PlaytestSessionStartRoute,
				FProtokitePlaytestFakeTransport::Status(200, ProtokitePlaytestFixtures::SessionStartBody()));
			Transport->Answer(ProtokitePlaytestFixtures::PlaytestSessionEndRoute, FProtokitePlaytestFakeTransport::NoContent());
			...
			if (bUseTestIdentitySources)
			{
				...
				const TSharedRef<FProtokitePlaytestRunningSteamAccount> Account = SteamAccount;
		...
		{
			Transport->Answer(ProtokitePlaytestFixtures::PlaytestConfigRoute, Response);
		}

		int32 ConfigRequests() const
		{
			return Transport->CountRequestsEndingWith(ProtokitePlaytestFixtures::PlaytestConfigRoute);
		}

		int32 SessionStarts() const
		{
			return Transport->CountRequestsEndingWith(ProtokitePlaytestFixtures::PlaytestSessionStartRoute);
		}
		...
		TSharedPtr<FJsonObject> LastSessionStartBody() const
		{
			...
			if (const FFlockHttpRequest* Request = Transport->FindLastRequestEndingWith(ProtokitePlaytestFixtures::PlaytestSessionStartRoute))
			{
		 ...
		 */
		void SignInToFlockOnStart(const FString& PlayerId = ProtokitePlaytestFixtures::FlockPlayerId)
		{
			...
			Transport->Answer(TEXT("/analytics/sessions"), FProtokitePlaytestFakeTransport::Status(200, FString::Printf(
				TEXT("{\"session_id\":\"%s\",\"event_name\":\"session_started\"}"), ProtokitePlaytestFixtures::FirstFlockSessionId)));
			Transport->Answer(TEXT("/analytics/events"), FProtokitePlaytestFakeTransport::Status(200, TEXT("{\"ok\":true,\"count\":1}")));
			Transport->Answer(TEXT("/log_event"), FProtokitePlaytestFakeTransport::Status(200, TEXT("{\"ok\":true,\"count\":1}")));
		}
			...
			for (const FFlockHttpRequest& Request : Transport->Requests)
			{
				...
				for (const TSharedPtr<FJsonValue>& Value : *Sent)
				{
					...
					if (Value.IsValid() && Value->TryGetObject(Event)
						&& StringMember(*Event, TEXT("event_category")).Equals(ProtokitePlaytestEvents::Category, ESearchCase::CaseSensitive))
					{
						Events.Add(*Event);
					}
				}
	...
	private:
		const FName PlaytestCategory = TEXT("LogProtokitePlaytest");
