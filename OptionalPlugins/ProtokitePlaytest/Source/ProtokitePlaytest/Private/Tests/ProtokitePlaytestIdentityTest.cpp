...
namespace
{
	...
	{
		FString Path = FPaths::Combine(FPaths::ProjectIntermediateDir(), TEXT("ProtokitePlaytestTests"),
	...
	void ExpectDeviceIdFileResult(FAutomationTestBase& Test, const FString& What, EProtokitePlaytestDeviceIdFileResult Actual,
		EProtokitePlaytestDeviceIdFileResult Expected)
	{
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestIdentityDeviceIdFileKeepsItsIdTest,
	"Protokite.Playtest.Identity.DeviceIdFileKeepsItsId",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestIdentityDeviceIdFileKeepsItsIdTest::RunTest(const FString& Parameters)
{
	FScopedTestFolder Folder;
	const FString Path = FPaths::Combine(Folder.Path, TEXT("device_id.txt"));

	FString First;
	ExpectDeviceIdFileResult(*this, TEXT("With no file yet, one is created"), FProtokitePlaytestDeviceIdFile(Path).ReadOrCreate(First),
		EProtokitePlaytestDeviceIdFileResult::Created);
	...
	FString Second;
	ExpectDeviceIdFileResult(*this, TEXT("The next launch reads the file"), FProtokitePlaytestDeviceIdFile(Path).ReadOrCreate(Second),
		EProtokitePlaytestDeviceIdFileResult::Read);
	TestEqual(TEXT("And gets the same id"), Second, First);
	TestEqual(TEXT("No temporary file is left behind"), Folder.Files().Num(), 1);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestIdentityFileWithoutADeviceIdIsReplacedTest,
	"Protokite.Playtest.Identity.FileWithoutADeviceIdIsReplaced",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestIdentityFileWithoutADeviceIdIsReplacedTest::RunTest(const FString& Parameters)
{
	...
	for (const FString& NotDeviceId : NotDeviceIds)
	{
		...
		ExpectDeviceIdFileResult(*this, What + TEXT(" is replaced"), FProtokitePlaytestDeviceIdFile(Path).ReadOrCreate(DeviceId),
			EProtokitePlaytestDeviceIdFileResult::Replaced);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestIdentityUnreadableFileIsLeftAloneTest,
	"Protokite.Playtest.Identity.UnreadableFileIsLeftAlone",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestIdentityUnreadableFileIsLeftAloneTest::RunTest(const FString& Parameters)
{
#if PLATFORM_WINDOWS
	FScopedTestFolder Folder;
	const FString Path = FPaths::Combine(Folder.Path, TEXT("device_id.txt"));
	FFileHelper::SaveStringToFile(SomeDeviceId, *Path, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM);
	{
		...
		{
			FString DeviceId;
			ExpectDeviceIdFileResult(*this, TEXT("A file that cannot be read"), FProtokitePlaytestDeviceIdFile(Path).ReadOrCreate(DeviceId),
				EProtokitePlaytestDeviceIdFileResult::Unreadable);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestIdentityDeviceIdThatCannotBeSavedIsNotUsedTest,
	"Protokite.Playtest.Identity.DeviceIdThatCannotBeSavedIsNotUsed",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestIdentityDeviceIdThatCannotBeSavedIsNotUsedTest::RunTest(const FString& Parameters)
{
	FScopedTestFolder Folder;
	...
	ExpectDeviceIdFileResult(*this, TEXT("An id that cannot be saved"),
		FProtokitePlaytestDeviceIdFile(FPaths::Combine(Blocker, TEXT("device_id.txt"))).ReadOrCreate(DeviceId),
		EProtokitePlaytestDeviceIdFileResult::CouldNotSave);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestIdentityFailedSaveGivesUpAtOnceTest,
	"Protokite.Playtest.Identity.FailedSaveGivesUpAtOnce",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestIdentityFailedSaveGivesUpAtOnceTest::RunTest(const FString& Parameters)
{
	FScopedTestFolder Folder;
	const FString Path = FPaths::Combine(Folder.Path, TEXT("device_id.txt"));
	...
	EProtokitePlaytestDeviceIdFileResult Result;
	int32 Complaints = 0;
	{
		FFileManagerComplaintCounter Counter;
		Result = FProtokitePlaytestDeviceIdFile(Path).ReadOrCreate(DeviceId);
		Complaints = Counter.Count();
	}
	const double Seconds = FPlatformTime::Seconds() - StartedAt;

	ExpectDeviceIdFileResult(*this, TEXT("An id that cannot be moved into place"), Result, EProtokitePlaytestDeviceIdFileResult::CouldNotSave);
	TestTrue(TEXT("Is not used"), DeviceId.IsEmpty());
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestIdentityStrayTemporaryFilesAreSweptTest,
	"Protokite.Playtest.Identity.StrayTemporaryFilesAreSwept",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestIdentityStrayTemporaryFilesAreSweptTest::RunTest(const FString& Parameters)
{
	FScopedTestFolder Folder;
	const FString Path = FPaths::Combine(Folder.Path, TEXT("device_id.txt"));
	FFileHelper::SaveStringToFile(SomeDeviceId, *Path, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM);
	...
	FString DeviceId;
	ExpectDeviceIdFileResult(*this, TEXT("The id is read as usual"), FProtokitePlaytestDeviceIdFile(Path).ReadOrCreate(DeviceId),
		EProtokitePlaytestDeviceIdFileResult::Read);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestIdentityUsableIdsTest,
	"Protokite.Playtest.Identity.UsableIds",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestIdentityUsableIdsTest::RunTest(const FString& Parameters)
{
	TestTrue(TEXT("A Steam id"), IsUsablePlaytestId(TEXT("76561198000000001"), ProtokitePlaytestIdentityLimits::SteamIdLength));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FProtokitePlaytestIdentityReadingSteamNeverCreatesItTest,
	"Protokite.Playtest.Identity.ReadingSteamNeverCreatesIt",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)

bool FProtokitePlaytestIdentityReadingSteamNeverCreatesItTest::RunTest(const FString& Parameters)
{
	...
	const FProtokitePlaytestRunningSteamAccount Account = ReadRunningSteamAccount();
