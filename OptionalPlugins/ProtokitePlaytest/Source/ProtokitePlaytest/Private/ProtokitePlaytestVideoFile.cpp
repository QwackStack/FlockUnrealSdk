...
namespace
{
	 ...
	 *
	 * The two copies of the name below sit in front of it, so **renaming the plugin moves it**: it went from 99 to 107
	 * when FlockPlaytest became ProtokitePlaytest. Open() checks the duration really is here before it records, because
	 * stamping eight bytes at the wrong offset on close writes a file no player can read.
	 */
	constexpr int64 DurationValueOffset = 107;
	...
	const ANSICHAR* const WrittenBy = "ProtokitePlaytest";
...
EProtokitePlaytestInterruptedVideoResult FProtokitePlaytestVideoFile::FinishInterruptedFile(const FString& UnfinishedPath, const FString& FinishedPath,
	int32& OutFramesKept, FString& OutError)
{
	...
	int32 Frames = 0;
	{
		...
		if (!Handle.IsValid())
		{
			...
			return EProtokitePlaytestInterruptedVideoResult::CouldNotFinish;
		}
	...
	if (Frames == 0)
	{
		...
		return EProtokitePlaytestInterruptedVideoResult::HeldNoFrame;
	}
	...
	return EProtokitePlaytestInterruptedVideoResult::Finished;
}

FProtokitePlaytestVideoFile::~FProtokitePlaytestVideoFile()
{
	Close();
}

bool FProtokitePlaytestVideoFile::Open(const FString& Path, FIntPoint FrameSize, FString& OutError)
{
	Close();
	BytesWritten = 0;
	FramesWritten = 0;
	...
	// Where the duration sits depends on how long the name written above is, and Close() stamps it in from the constant
	// rather than from this header. Read back off the bytes just built: the duration's own id, then its one-byte size.
	const bool bDurationWhereClosingWillStampIt = DurationValueOffset >= 3 && DurationValueOffset + 8 <= Header.Num()
		&& Header[DurationValueOffset - 3] == 0x44 && Header[DurationValueOffset - 2] == 0x89
		&& Header[DurationValueOffset - 1] == 0x88;
	if (!bDurationWhereClosingWillStampIt)
	{
		OutError = FString::Printf(TEXT("the WebM duration is not at byte %lld, which is where this build stamps it in when the file closes"),
			DurationValueOffset);
		Archive.Reset();
		return false;
	}
...
bool FProtokitePlaytestVideoFile::WriteFrame(const TArray<uint8>& FrameBytes, int64 TimestampMs, bool bKeyFrame)
{
	if (!Archive.IsValid())
	{
		return false;
...
void FProtokitePlaytestVideoFile::AbandonForTesting()
{
	if (Archive.IsValid())
	{
...
bool FProtokitePlaytestVideoFile::Close()
{
	if (!Archive.IsValid())
	{
		return true;
