...
FString SProtokitePlaytestFormWidget::GetIconPath()
{
	const TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("ProtokitePlaytest"));
	return Plugin.IsValid() ? FPaths::Combine(Plugin->GetBaseDir(), TEXT("Resources"), TEXT("FeedbackFormIcon.png")) : FString();
}

void SProtokitePlaytestFormWidget::Construct(const FArguments& InArgs)
{
			...
			[
				...
				[
					...
					[
						SNew(SButton)
						.HAlign(HAlign_Center)
						.Visibility(this, &SProtokitePlaytestFormWidget::GetSendRecordingVisibility)
						.OnClicked(this, &SProtokitePlaytestFormWidget::SendTheRecording)
					...
					[
						...
						.Visibility(this, &SProtokitePlaytestFormWidget::GetRecordingOnItsWayVisibility)
						.Font(Font(10))
						.ColorAndOpacity(HelpColour)
						.AutoWrapText(true)
					]
...
TSharedRef<SWidget> SProtokitePlaytestFormWidget::BuildField(const FProtokitePlaytestFormField& Field)
{
	...
	return SNew(SVerticalBox)
		...
		[
			SNew(STextBlock)
			.Text(this, &SProtokitePlaytestFormWidget::GetProblemText, FieldId)
			.Visibility(this, &SProtokitePlaytestFormWidget::GetProblemVisibility, FieldId)
			.Font(Font(10))
...
TSharedRef<SWidget> SProtokitePlaytestFormWidget::BuildAnswerControl(const FProtokitePlaytestFormField& Field)
{
	const FString FieldId = Field.Id;

	if (Field.IsOfKind(ProtokitePlaytestFormFieldTypes::TextArea))
	{
...
TSharedRef<SWidget> SProtokitePlaytestFormWidget::BuildRating(const FProtokitePlaytestFormField& Field)
{
	const FString FieldId = Field.Id;
	const TSharedRef<SHorizontalBox> Row = SNew(SHorizontalBox);

	for (int32 Score = ProtokitePlaytestRatings::Lowest; Score <= ProtokitePlaytestRatings::Highest; ++Score)
	{
...
TSharedRef<SWidget> SProtokitePlaytestFormWidget::BuildSelect(const FProtokitePlaytestFormField& Field)
{
	const FString FieldId = Field.Id;
...
FText SProtokitePlaytestFormWidget::GetProblemText(FString FieldId) const
{
	for (const FProtokitePlaytestFormProblem& Problem : Problems)
	{
...
EVisibility SProtokitePlaytestFormWidget::GetProblemVisibility(FString FieldId) const
{
	return GetProblemText(FieldId).IsEmpty() ? EVisibility::Collapsed : EVisibility::Visible;
}

TArray<FProtokitePlaytestFormProblem> SProtokitePlaytestFormWidget::TrySubmit()
{
...
EVisibility SProtokitePlaytestFormWidget::GetSendRecordingVisibility() const
{
...
EVisibility SProtokitePlaytestFormWidget::GetRecordingOnItsWayVisibility() const
{
...
FReply SProtokitePlaytestFormWidget::SendTheRecording()
{
