...
struct FProtokitePlaytestEncodedFrame
{
	TArray<uint8> Bytes;
	int64 TimestampMs = 0;
 ...
 */
class FProtokitePlaytestVideoEncoder
{
public:
	FProtokitePlaytestVideoEncoder();
	~FProtokitePlaytestVideoEncoder();

	FProtokitePlaytestVideoEncoder(const FProtokitePlaytestVideoEncoder&) = delete;
	FProtokitePlaytestVideoEncoder& operator=(const FProtokitePlaytestVideoEncoder&) = delete;
	 ...
	 */
	bool Encode(const TArray<FColor>& Pixels, int64 TimestampMs, int64 DurationMs, TArray<FProtokitePlaytestEncodedFrame>& OutFrames, FString& OutError);

	/** Hands over anything the encoder still holds. Nothing can be encoded afterwards. */
	bool Finish(TArray<FProtokitePlaytestEncodedFrame>& OutFrames, FString& OutError);
