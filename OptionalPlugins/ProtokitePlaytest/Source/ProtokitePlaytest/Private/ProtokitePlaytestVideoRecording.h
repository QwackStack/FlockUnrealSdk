...
struct FProtokitePlaytestVideoRecordingSummary
{
	/** The finished file. Empty when none was kept: no whole frame was written, or the file could not even be finished. */
	FString FilePath;
	EProtokitePlaytestVideoStopReason StopReason = EProtokitePlaytestVideoStopReason::StoppedByGame;
...
public:
	 ...
	 */
	static TSharedPtr<FProtokitePlaytestVideoRecording> Start(const TSharedRef<IProtokitePlaytestVideoFrameSource>& Source,
		const FProtokitePlaytestVideoSettings& Settings, const FString& FilePath, FString& OutError,
	...
	~FProtokitePlaytestVideoRecording();

	FProtokitePlaytestVideoRecording(const FProtokitePlaytestVideoRecording&) = delete;
	FProtokitePlaytestVideoRecording& operator=(const FProtokitePlaytestVideoRecording&) = delete;
	 ...
	 */
	TOptional<EProtokitePlaytestVideoStopReason> AddFrame(double FrameSeconds);
	...
	void StopCapturing(EProtokitePlaytestVideoStopReason Reason);
	...
	FProtokitePlaytestVideoRecordingSummary GetSummary() const;

private:
	struct FWriter;
	struct FWorkerThread;

	FProtokitePlaytestVideoRecording(const TSharedRef<IProtokitePlaytestVideoFrameSource>& InSource, const TSharedRef<FWriter>& InWriter,
		const FProtokitePlaytestVideoSettings& Settings);

	void SendFramesToWriter(TArray<FProtokitePlaytestVideoFrame>&& Frames);

	TSharedRef<IProtokitePlaytestVideoFrameSource> Source;
	TSharedRef<FWriter> Writer;
	FProtokitePlaytestVideoFrameSchedule Schedule;
