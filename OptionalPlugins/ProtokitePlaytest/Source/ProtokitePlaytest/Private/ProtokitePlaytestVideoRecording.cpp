...
FString DescribeVideoStopReason(EProtokitePlaytestVideoStopReason Reason)
{
	switch (Reason)
	{
	case EProtokitePlaytestVideoStopReason::StoppedByGame: return TEXT("the game stopped it");
	case EProtokitePlaytestVideoStopReason::ReachedLengthLimit: return TEXT("it reached its length limit");
	case EProtokitePlaytestVideoStopReason::ReachedSizeLimit: return TEXT("it reached its size limit");
	case EProtokitePlaytestVideoStopReason::PlaytestStopped: return TEXT("playtesting stopped");
	case EProtokitePlaytestVideoStopReason::GameInstanceShutDown: return TEXT("the game instance shut down");
	case EProtokitePlaytestVideoStopReason::CouldNotWrite: return TEXT("its file could not be written");
	}
 ...
 */
struct FProtokitePlaytestVideoRecording::FWorkerThread : public FRunnable
{
 ...
 * file thread.
 */
struct FProtokitePlaytestVideoRecording::FWriter : public TSharedFromThis<FProtokitePlaytestVideoRecording::FWriter>
{
	FProtokitePlaytestVideoEncoder Encoder;
	FProtokitePlaytestVideoFile File;
	FProtokitePlaytestVideoSettings Settings;
	...
	int64 BytesHandedToFile = FProtokitePlaytestVideoFile::FileHeaderBytes;
	...
	void EncodeFrame(const FProtokitePlaytestVideoFrame& Frame)
	{
		if (bCouldNotWrite)
		{
			return;
	...
	void HandFramesToFile(TArray<FProtokitePlaytestEncodedFrame>&& Encoded)
	{
		for (FProtokitePlaytestEncodedFrame& Frame : Encoded)
		{
			// Once a frame has been refused, no later one is written either, so the video ends there rather than skipping it.
			if (bSizeLimitReached || BytesHandedToFile + FProtokitePlaytestVideoFile::FrameHeaderBytes + Frame.Bytes.Num() > Settings.MaxBytes)
			{
				bSizeLimitReached = true;
				return;
			}
			BytesHandedToFile += FProtokitePlaytestVideoFile::FrameHeaderBytes + Frame.Bytes.Num();
	...
	void Finish()
	{
		...
		if (bCouldNotWrite)
		{
			...
			int32 FramesKept = 0;
			FString FinishError;
			switch (FProtokitePlaytestVideoFile::FinishInterruptedFile(PartPath, FinishedPath, FramesKept, FinishError))
			{
			case EProtokitePlaytestInterruptedVideoResult::Finished:
				KeptPath = FinishedPath;
				FramesInKeptFile = FramesKept;
				BytesInKeptFile = IFileManager::Get().FileSize(*FinishedPath);
				break;
			case EProtokitePlaytestInterruptedVideoResult::HeldNoFrame:
				break;
			case EProtokitePlaytestInterruptedVideoResult::CouldNotFinish:
...
TSharedPtr<FProtokitePlaytestVideoRecording> FProtokitePlaytestVideoRecording::Start(const TSharedRef<IProtokitePlaytestVideoFrameSource>& Source,
	const FProtokitePlaytestVideoSettings& Settings, const FString& FilePath, FString& OutError, TFunction<void()> BeforeEachEncodeForTesting,
	TFunction<bool()> BeforeEachWriteForTesting)
{
	...
	return MakeShareable(new FProtokitePlaytestVideoRecording(Source, Writer, Settings));
}

FProtokitePlaytestVideoRecording::FProtokitePlaytestVideoRecording(const TSharedRef<IProtokitePlaytestVideoFrameSource>& InSource,
	const TSharedRef<FWriter>& InWriter, const FProtokitePlaytestVideoSettings& Settings)
	: Source(InSource)
	, Writer(InWriter)
	, Schedule(Settings.FramesPerSecond, Settings.MaxSeconds)
	, FileThread(MakeUnique<FWorkerThread>(TEXT("ProtokitePlaytestVideoFileWriter")))
	, EncodingThread(MakeUnique<FWorkerThread>(TEXT("ProtokitePlaytestVideoEncoder")))
{
	Writer->FileThread = FileThread.Get();
}

FProtokitePlaytestVideoRecording::~FProtokitePlaytestVideoRecording()
{
	StopCapturing(EProtokitePlaytestVideoStopReason::GameInstanceShutDown);
	WaitUntilWritten();
}

TOptional<EProtokitePlaytestVideoStopReason> FProtokitePlaytestVideoRecording::AddFrame(double FrameSeconds)
{
	if (!bCapturing)
	{
		return {};
	...
	TArray<FProtokitePlaytestVideoFrame> Arrived;
	Source->TakeCapturedFrames(Arrived);
	SendFramesToWriter(MoveTemp(Arrived));

	if (Writer->bCouldNotWrite)
	{
		return EProtokitePlaytestVideoStopReason::CouldNotWrite;
	}
	if (Writer->bSizeLimitReached)
	{
		return EProtokitePlaytestVideoStopReason::ReachedSizeLimit;
	}

	int64 TimestampMs = 0;
	switch (Schedule.AddFrame(FrameSeconds, TimestampMs))
	{
	case FProtokitePlaytestVideoFrameSchedule::EFrameResult::ReachedLengthLimit:
		return EProtokitePlaytestVideoStopReason::ReachedLengthLimit;
	case FProtokitePlaytestVideoFrameSchedule::EFrameResult::Capture:
		...
		break;
	case FProtokitePlaytestVideoFrameSchedule::EFrameResult::Skip:
		break;
	}
	return {};
}

void FProtokitePlaytestVideoRecording::StopCapturing(EProtokitePlaytestVideoStopReason Reason)
{
	if (!bCapturing)
	{
		return;
	...
	TArray<FProtokitePlaytestVideoFrame> Remaining;
...
bool FProtokitePlaytestVideoRecording::HasFinishedWriting() const
{
	return Writer->bWritten.load();
}

void FProtokitePlaytestVideoRecording::WaitUntilWritten()
{
...
FProtokitePlaytestVideoRecordingSummary FProtokitePlaytestVideoRecording::GetSummary() const
{
	FProtokitePlaytestVideoRecordingSummary Summary;
...
void FProtokitePlaytestVideoRecording::SendFramesToWriter(TArray<FProtokitePlaytestVideoFrame>&& Frames)
{
	for (FProtokitePlaytestVideoFrame& Frame : Frames)
	{
		...
		const TSharedRef<FProtokitePlaytestVideoFrame> Shared = MakeShared<FProtokitePlaytestVideoFrame>(MoveTemp(Frame));
