...
FProtokitePlaytestRunningSteamAccount ReadRunningSteamAccount()
{
	FProtokitePlaytestRunningSteamAccount Account;
...
{
	return !Id.IsEmpty() && Id.Len() <= MaxLength && !ProtokitePlaytestText::ContainsWhitespace(Id);
}

FProtokitePlaytestDeviceIdFile::FProtokitePlaytestDeviceIdFile(const FString& InPath)
	: Path(InPath)
{
}

FString FProtokitePlaytestDeviceIdFile::GetDefaultPath()
{
	return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("ProtokitePlaytest"), TEXT("device_id.txt"));
}

EProtokitePlaytestDeviceIdFileResult FProtokitePlaytestDeviceIdFile::ReadOrCreate(FString& OutDeviceId) const
{
	OutDeviceId.Empty();
	...
	{
		return SaveNewDeviceId(OutDeviceId) ? EProtokitePlaytestDeviceIdFileResult::Created : EProtokitePlaytestDeviceIdFileResult::CouldNotSave;
	}
	...
	if (!FFileHelper::LoadFileToString(Contents, *Path))
	{
		...
		return EProtokitePlaytestDeviceIdFileResult::Unreadable;
	}
	if (IsDeviceId(Contents))
	{
		OutDeviceId = Contents;
		return EProtokitePlaytestDeviceIdFileResult::Read;
	}
	return SaveNewDeviceId(OutDeviceId) ? EProtokitePlaytestDeviceIdFileResult::Replaced : EProtokitePlaytestDeviceIdFileResult::CouldNotSave;
}

bool FProtokitePlaytestDeviceIdFile::SaveNewDeviceId(FString& OutDeviceId) const
{
...
void FProtokitePlaytestDeviceIdFile::SweepStrayTemporaryFiles() const
{
