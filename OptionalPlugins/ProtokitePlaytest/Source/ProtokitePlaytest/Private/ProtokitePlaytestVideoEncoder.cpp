...
#if WITH_PROTOKITE_PLAYTEST_VIDEO
struct FProtokitePlaytestVideoEncoder::FLibVpxState
{
...
#if WITH_PROTOKITE_PLAYTEST_VIDEO
	void CollectEncodedFrames(vpx_codec_ctx_t& Codec, TArray<FProtokitePlaytestEncodedFrame>& OutFrames)
	{
		...
		while (const vpx_codec_cx_pkt_t* Packet = vpx_codec_get_cx_data(&Codec, &Iterator))
		{
			...
			FProtokitePlaytestEncodedFrame& Frame = OutFrames.AddDefaulted_GetRef();
...
FProtokitePlaytestVideoEncoder::FProtokitePlaytestVideoEncoder()
...
bool FProtokitePlaytestVideoEncoder::IsBuiltWithVideo()
{
	return WITH_PROTOKITE_PLAYTEST_VIDEO != 0;
}

bool FProtokitePlaytestVideoEncoder::Initialize(FIntPoint InFrameSize, int32 FramesPerSecond, int32 BitrateKbps, FString& OutError)
{
#if WITH_PROTOKITE_PLAYTEST_VIDEO
	if (State->bInitialized)
	{
...
bool FProtokitePlaytestVideoEncoder::Encode(const TArray<FColor>& Pixels, int64 TimestampMs, int64 DurationMs,
	TArray<FProtokitePlaytestEncodedFrame>& OutFrames, FString& OutError)
{
#if WITH_PROTOKITE_PLAYTEST_VIDEO
	if (!State->bInitialized)
	{
...
bool FProtokitePlaytestVideoEncoder::Finish(TArray<FProtokitePlaytestEncodedFrame>& OutFrames, FString& OutError)
{
#if WITH_PROTOKITE_PLAYTEST_VIDEO
	if (!State->bInitialized)
	{
...
void FProtokitePlaytestVideoEncoder::ConvertToVideoColours(const TArray<FColor>& Pixels, FIntPoint InFrameSize, TArray<uint8>& OutPlanes)
{
