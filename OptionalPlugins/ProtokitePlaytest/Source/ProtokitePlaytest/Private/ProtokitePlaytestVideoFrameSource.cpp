...
namespace
{
	...
	void MoveFrames(TArray<FCapturedFrameData>&& Captured, TArray<FProtokitePlaytestVideoFrame>& OutFrames)
	{
		for (FCapturedFrameData& Data : Captured)
		{
			...
			FProtokitePlaytestVideoFrame& Frame = OutFrames.AddDefaulted_GetRef();
...
TSharedPtr<IProtokitePlaytestVideoFrameSource> FProtokitePlaytestGameViewportFrameSource::Create(const UGameInstance* GameInstance,
	FIntPoint MaxVideoSize, FString& OutWhyNot)
{
	OutWhyNot = DecideWhyVideoCannotBeRecorded(FProtokitePlaytestVideoEncoder::IsBuiltWithVideo(), FApp::CanEverRender(),
	...
	return MakeShareable(new FProtokitePlaytestGameViewportFrameSource(Viewport.ToSharedRef(), FrameSize));
}

FProtokitePlaytestGameViewportFrameSource::FProtokitePlaytestGameViewportFrameSource(const TSharedRef<FSceneViewport>& InViewport, FIntPoint InFrameSize)
	: Viewport(InViewport)
...
FProtokitePlaytestGameViewportFrameSource::~FProtokitePlaytestGameViewportFrameSource()
{
	TArray<FProtokitePlaytestVideoFrame> Dropped;
	Stop(Dropped);
}

bool FProtokitePlaytestGameViewportFrameSource::IsReadyForAnotherFrame() const
{
...
void FProtokitePlaytestGameViewportFrameSource::CaptureFrame(int64 TimestampMs)
{
...
void FProtokitePlaytestGameViewportFrameSource::TakeCapturedFrames(TArray<FProtokitePlaytestVideoFrame>& OutFrames)
{
...
void FProtokitePlaytestGameViewportFrameSource::Stop(TArray<FProtokitePlaytestVideoFrame>& OutFrames)
{
	if (bStopped)
	{
		return;
...
void FProtokitePlaytestGameViewportFrameSource::StartGrabber(const TSharedRef<FSceneViewport>& InViewport)
{
...
void FProtokitePlaytestGameViewportFrameSource::StopGrabber(TArray<FProtokitePlaytestVideoFrame>& OutFrames)
{
	if (!Grabber.IsValid())
	{
		return;
