...
class UProtokitePlaytestSettings;

/** The video settings one recording uses: Protokite Playtest Settings' Video Recording values, kept in range. */
struct FProtokitePlaytestVideoSettings
{
	...
	static FProtokitePlaytestVideoSettings FromProjectSettings(const UProtokitePlaytestSettings& Settings);
 ...
 */
class FProtokitePlaytestVideoFrameSchedule
{
public:
	...
	FProtokitePlaytestVideoFrameSchedule(int32 FramesPerSecond, double MaxSeconds);
