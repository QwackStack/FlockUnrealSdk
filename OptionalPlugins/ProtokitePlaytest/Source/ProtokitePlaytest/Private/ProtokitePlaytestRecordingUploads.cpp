...
FProtokitePlaytestRecordingUploads::FProtokitePlaytestRecordingUploads(const TSharedRef<FProtokiteClient>& InClient,
...
TMap<FString, FString> FProtokitePlaytestRecordingUploads::MakeUploadHeaders(const TMap<FString, FString>& LaunchHeaders,
...
void FProtokitePlaytestRecordingUploads::UploadOne(const TSharedRef<FProtokitePlaytestRecordingRun>& Run,
	const FProtokitePlaytestRecordingSession& Session, const TMap<FString, FString>& LaunchHeaders,
	TFunction<void(FProtokitePlaytestRecordingUploadOutcome)> OnDone)
{
	FProtokitePlaytestRecordingUploadOutcome Refused;
...
void FProtokitePlaytestRecordingUploads::AskForLinkAndSend(const TSharedRef<FProtokitePlaytestRecordingRun>& Run,
	const FProtokitePlaytestRecordingSession& Session, const TMap<FString, FString>& UploadHeaders, int32 AttemptsSoFar,
	TFunction<void(FProtokitePlaytestRecordingUploadOutcome)> OnDone)
{
	...
	TWeakPtr<FProtokitePlaytestRecordingUploads> WeakSelf = AsShared();

	Client->RequestRecordingUploadLink(Session.ProtokiteApiUrl, UploadHeaders, Session.ProtokiteSessionId,
		ProtokitePlaytestRecordingContentTypes::WebM,
		[WeakSelf, Run, Session, UploadHeaders, Attempt, VideoFilePath, OnDone]
		(TFlockResult<FProtokitePlaytestRecordingUploadLink> Link)
		{
			const TSharedPtr<FProtokitePlaytestRecordingUploads> Self = WeakSelf.Pin();
			if (!Self.IsValid())
			{
				return;
			}

			FProtokitePlaytestRecordingUploadOutcome Outcome;
			...
			Self->Uploader->UploadFile(Link.Value.UploadUrl, VideoFilePath, ProtokitePlaytestRecordingContentTypes::WebM,
				Self->UploadTimeoutSeconds, nullptr,
				[WeakSelf, Run, Session, UploadHeaders, Attempt, OnDone](FFlockFileUploadOutcome Sent)
				{
					const TSharedPtr<FProtokitePlaytestRecordingUploads> SelfAgain = WeakSelf.Pin();
...
void FProtokitePlaytestRecordingUploads::UploadEveryOneWaiting(const FString& RecordingsFolder,
	const TMap<FString, FString>& LaunchHeaders, TFunction<void(int32, int32)> OnDone)
{
	const TArray<FProtokitePlaytestRecordingWaitingToUpload> Waiting =
		FProtokitePlaytestRecordingsFolder::FindRecordingsWaitingToUpload(RecordingsFolder);
	...
	const TSharedRef<TArray<FProtokitePlaytestRecordingWaitingToUpload>> ToUpload =
		MakeShared<TArray<FProtokitePlaytestRecordingWaitingToUpload>>(Waiting);
	...
	*UploadNext = [WeakSelf, ToUpload, Uploaded, Left, Next, LaunchHeaders, UploadNext, OnDone]()
	{
		...
		{
			const FProtokitePlaytestRecordingWaitingToUpload Recording = (*ToUpload)[(*Next)++];

			// Taking hold is what says no other launch is uploading this one. One it holds is left alone.
			const TSharedPtr<FProtokitePlaytestRecordingRun> Run = FProtokitePlaytestRecordingRun::ClaimEnded(Recording.RunFolder);
			if (!Run.IsValid())
			{
				++(*Left);
				continue;
			...
			Self->UploadOne(Run.ToSharedRef(), Recording.Session, LaunchHeaders,
				[Uploaded, Left, UploadNext](FProtokitePlaytestRecordingUploadOutcome Outcome)
				{
