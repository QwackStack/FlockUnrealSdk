...
struct FFlockPlaytestConsentOption
{
 ...
 */
class SFlockPlaytestConsentWidget : public SCompoundWidget
{
public:
	DECLARE_DELEGATE_OneParam(FOnConsentChosen, EFlockPlaytestConsentChoice);

	SLATE_BEGIN_ARGS(SFlockPlaytestConsentWidget) {}
	...
	static TArrayView<const FFlockPlaytestConsentOption> GetOptions();

	/** Answers as pressing that option's button does. */
	void ChooseForTesting(EFlockPlaytestConsentChoice Choice) { Choose(Choice); }

private:
	/** Hands the answer over. The one place an answer leaves this widget. */
	FReply Choose(EFlockPlaytestConsentChoice Choice);

	TSharedRef<SWidget> BuildOption(const FFlockPlaytestConsentOption& Option);
