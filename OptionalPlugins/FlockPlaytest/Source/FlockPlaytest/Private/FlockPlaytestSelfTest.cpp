...
namespace
{
	...
	const TCHAR* DescribeSelfTestOutcome(EFlockPlaytestSelfTestOutcome Outcome)
	{
		switch (Outcome)
		{
		case EFlockPlaytestSelfTestOutcome::Passed:
			return TEXT("PASS");
		case EFlockPlaytestSelfTestOutcome::Skipped:
			return TEXT("SKIPPED");
		default:
			return TEXT("FAIL");
		}
	...
	FString DescribeSelfTestPlaytestStatus(EFlockPlaytestStatus Status)
	{
		return StaticEnum<EFlockPlaytestStatus>()->GetNameStringByValue(static_cast<int64>(Status));
	}

	FString SerializeSelfTestAnswers(const FFlockPlaytestFormAnswers& Answers, const FFlockPlaytestForm& Form)
	{
		FString Json;
	...
	const TCHAR* SelfTestAnswerText = TEXT("Sent by FlockPlaytest.SelfTest.");
	...
	bool RaiseTheSameBlueprintFaultTwice(UObject* Context, const FString& RunId)
	{
		...
		const FBlueprintExceptionInfo Info(EBlueprintExceptionType::AccessViolation, FText::FromString(FString::Printf(
			TEXT("Accessed None trying to read property FlockPlaytestSelfTestTarget (raised by FlockPlaytest.SelfTest, run %s)"), *RunId)));
...
TSharedRef<FFlockPlaytestSelfTest> FFlockPlaytestSelfTest::Start(UFlockPlaytestSubsystem* InPlaytest, UFlockSubsystem* InFlock,
	const FOptions& InOptions, TFunction<void(const TArray<FFlockPlaytestSelfTestStep>&)> InOnFinished)
{
	const TSharedRef<FFlockPlaytestSelfTest> Run = MakeShareable(new FFlockPlaytestSelfTest());
	...
	Run->Client = MakeShared<FFlockProtokiteClient>(HttpClient, NoRetries, Logger);

	using namespace FlockPlaytestSelfTestSteps;
	Run->StepsToRun = {
		{ PlaytestLoaded, ENeeds::Nothing, &FFlockPlaytestSelfTest::CheckPlaytestLoaded },
		{ WrongApiKeyRefused, ENeeds::Playtest, &FFlockPlaytestSelfTest::CheckWrongApiKeyRefused },
		{ MissingApiKeyRefused, ENeeds::Playtest, &FFlockPlaytestSelfTest::CheckMissingApiKeyRefused },
		{ VersionWithNoPlaytestRefused, ENeeds::Playtest, &FFlockPlaytestSelfTest::CheckVersionWithNoPlaytestRefused },
		{ SessionStarted, ENeeds::Playtest, &FFlockPlaytestSelfTest::CheckSessionStarted },
		{ SessionWithNoPlayerRefused, ENeeds::Playtest, &FFlockPlaytestSelfTest::CheckSessionWithNoPlayerRefused },
		{ ClosedPlaytestRefused, ENeeds::Session, &FFlockPlaytestSelfTest::CheckClosedPlaytestRefused },
		{ ExceptionReported, ENeeds::Playtest, &FFlockPlaytestSelfTest::CheckExceptionReported },
		{ PlaytestEventRecorded, ENeeds::Session, &FFlockPlaytestSelfTest::CheckPlaytestEventRecorded },
		{ FormMissingANeededAnswerRefused, ENeeds::Session, &FFlockPlaytestSelfTest::CheckFormMissingANeededAnswerRefused },
		{ FormWithAnOptionNotOnTheListRefused, ENeeds::Session, &FFlockPlaytestSelfTest::CheckFormWithAnOptionNotOnTheListRefused },
		{ FormForASessionThatDoesNotExistRefused, ENeeds::Session, &FFlockPlaytestSelfTest::CheckFormForASessionThatDoesNotExistRefused },
		{ FormTaken, ENeeds::Session, &FFlockPlaytestSelfTest::CheckFormTaken },
		{ UploadLinkForASessionThatDoesNotExistRefused, ENeeds::Playtest, &FFlockPlaytestSelfTest::CheckUploadLinkForASessionThatDoesNotExistRefused },
		{ RecordingUploaded, ENeeds::Session, &FFlockPlaytestSelfTest::CheckRecordingUploaded },
		{ EndForASessionThatDoesNotExistRefused, ENeeds::Playtest, &FFlockPlaytestSelfTest::CheckEndForASessionThatDoesNotExistRefused },
		{ SessionEnded, ENeeds::Session, &FFlockPlaytestSelfTest::CheckSessionEnded },
	};

	UE_LOG(LogFlockPlaytest, Display, TEXT("Playtest self-test: starting, against %s."), *Run->ProtokiteApiUrl());
	Run->RunNextStep();
	return Run;
}

FFlockPlaytestSelfTest::~FFlockPlaytestSelfTest()
{
	StopWaiting();
	StopListeningForTheUpload();
}

bool FFlockPlaytestSelfTest::IsTheExpectedRefusal(bool bSucceeded, const FFlockError& Error, int32 ExpectedStatus,
	const FString& QuestionItMustName, FString& OutWhatHappened)
{
	...
	if (!QuestionItMustName.IsEmpty())
	{
		...
		const FString Named = FlockPlaytestFindFieldIdInComplaint(Reason);
...
FString FFlockPlaytestSelfTest::MakeOptionNotOnTheList(const TArray<FString>& Options)
{
...
void FFlockPlaytestSelfTest::RunNextStep()
{
	...
	{
		Finish(EFlockPlaytestSelfTestOutcome::Skipped, TEXT("the playtest config did not load"));
		return;
	}
	if (Step.Needs == ENeeds::Session && !bSessionStarted)
	{
		Finish(EFlockPlaytestSelfTestOutcome::Skipped, TEXT("this launch's Protokite session did not start"));
		return;
	}
	(this->*Step.Run)();
}

void FFlockPlaytestSelfTest::Finish(EFlockPlaytestSelfTestOutcome Outcome, const FString& Detail)
{
	...
	if (Outcome == EFlockPlaytestSelfTestOutcome::Failed)
	{
		UE_LOG(LogFlockPlaytest, Warning, TEXT("Playtest self-test: %-7s %s -- %s"), DescribeSelfTestOutcome(Outcome), *Name, *Detail);
	}
	else
	{
		UE_LOG(LogFlockPlaytest, Display, TEXT("Playtest self-test: %-7s %s -- %s"), DescribeSelfTestOutcome(Outcome), *Name, *Detail);
	}
	RunNextStep();
}

void FFlockPlaytestSelfTest::FinishBecauseTheGameShutDown()
{
	const bool bFirstToNotice = !bGameShutDown;
	bGameShutDown = true;
	Finish(bFirstToNotice ? EFlockPlaytestSelfTestOutcome::Failed : EFlockPlaytestSelfTestOutcome::Skipped,
		TEXT("the game shut down during the self-test"));
}

void FFlockPlaytestSelfTest::FinishWithRefusal(bool bSucceeded, const FFlockError& Error, int32 ExpectedStatus,
	const FString& QuestionItMustName)
{
	...
	Finish(bExpected ? EFlockPlaytestSelfTestOutcome::Passed : EFlockPlaytestSelfTestOutcome::Failed, WhatHappened);
}

void FFlockPlaytestSelfTest::WaitUntil(TFunction<bool()> Condition, float Seconds, TFunction<void(bool bHappened)> Then)
{
	if (Condition())
	{
		Then(true);
	...
	const TWeakPtr<FFlockPlaytestSelfTest> WeakSelf = AsShared();
	...
	WaitHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateLambda(
		...
		{
			const TSharedPtr<FFlockPlaytestSelfTest> Self = WeakSelf.Pin();
			if (!Self.IsValid())
			{
				return false;
			}
...
void FFlockPlaytestSelfTest::StopWaiting()
{
...
void FFlockPlaytestSelfTest::Report()
{
	bFinished = true;
	...
	{
		Passed += Step.Outcome == EFlockPlaytestSelfTestOutcome::Passed ? 1 : 0;
		Failed += Step.Outcome == EFlockPlaytestSelfTestOutcome::Failed ? 1 : 0;
		Skipped += Step.Outcome == EFlockPlaytestSelfTestOutcome::Skipped ? 1 : 0;
	}
	if (Failed > 0)
	{
		UE_LOG(LogFlockPlaytest, Warning, TEXT("Playtest self-test finished: %d passed, %d failed, %d skipped."), Passed, Failed, Skipped);
	}
	else
	{
		UE_LOG(LogFlockPlaytest, Display, TEXT("Playtest self-test finished: %d passed, %d failed, %d skipped."), Passed, Failed, Skipped);
	}

	if (OnFinished)
	{
		const TFunction<void(const TArray<FFlockPlaytestSelfTestStep>&)> Callback = MoveTemp(OnFinished);
		Callback(Steps);
	}
}

void FFlockPlaytestSelfTest::StopListeningForTheUpload()
{
	if (Listener == nullptr)
	{
		return;
	}
	if (UFlockPlaytestSubsystem* PlaytestNow = Playtest.Get())
	{
		PlaytestNow->OnRecordingUploadFinished.RemoveDynamic(Listener, &UFlockPlaytestSelfTestListener::HandleRecordingUploadFinished);
	}
...
TMap<FString, FString> FFlockPlaytestSelfTest::Headers() const
{
...
FString FFlockPlaytestSelfTest::ProtokiteApiUrl() const
{
	return GetDefault<UFlockPlaytestSettings>()->ProtokiteApiUrl;
}

FFlockPlaytestFormSubmission FFlockPlaytestSelfTest::MakeFormSubmission(const FString& PlaytestSessionId, const FString& AnswersJson) const
{
	const UFlockPlaytestSubsystem* PlaytestNow = Playtest.Get();
...
void FFlockPlaytestSelfTest::CheckPlaytestLoaded()
{
	...
	const TWeakObjectPtr<UFlockPlaytestSubsystem> WeakPlaytest = Playtest;
	WaitUntil([WeakPlaytest]()
		{
			...
			const EFlockPlaytestStatus Status = WeakPlaytest->GetStatus();
			return Status != EFlockPlaytestStatus::WaitingForFlock && Status != EFlockPlaytestStatus::FetchingPlaytestConfig
				&& Status != EFlockPlaytestStatus::PlaytestConfigUnavailable;
		},
		Options.WaitSeconds, [this](bool)
		{
			...
			if (PlaytestNow->GetStatus() != EFlockPlaytestStatus::Ready)
			{
				Finish(EFlockPlaytestSelfTestOutcome::Failed, FString::Printf(TEXT("the playtest status is %s; the log above says "
					"why. Enable Playtesting and Protokite API URL are in Project Settings > Plugins > Flock Playtest Settings."),
					*DescribeSelfTestPlaytestStatus(PlaytestNow->GetStatus())));
				return;
			}

			bPlaytestLoaded = true;
			const FFlockPlaytestConfig& Config = PlaytestNow->GetPlaytestConfig();
			const auto OnOrOff = [PlaytestNow](const TCHAR* Feature) { return PlaytestNow->IsPlaytestFeatureEnabled(Feature) ? TEXT("on") : TEXT("off"); };
			Finish(EFlockPlaytestSelfTestOutcome::Passed, FString::Printf(TEXT("playtest %s for version %s; video recording %s, "
				"heavy analytics %s, exception capturing %s; %s"), *Config.TestId, *Config.FlockGameVersionId,
				OnOrOff(FlockPlaytestFeatures::VideoRecording), OnOrOff(FlockPlaytestFeatures::HeavyAnalytics),
				OnOrOff(FlockPlaytestFeatures::ExceptionCapturing),
				Config.HasForm() ? *FString::Printf(TEXT("a feedback form of %d questions"), Config.Form.Fields.Num()) : TEXT("no feedback form")));
		});
}

void FFlockPlaytestSelfTest::CheckWrongApiKeyRefused()
{
	TMap<FString, FString> SentHeaders = Headers();
	SentHeaders.Add(TEXT("X-Flock-API-Key"), WrongApiKey);
	const TWeakPtr<FFlockPlaytestSelfTest> WeakSelf = AsShared();
	Client->FetchPlaytestConfig(ProtokiteApiUrl(), SentHeaders, [WeakSelf](TFlockResult<FFlockPlaytestConfig> Result)
		{
			if (const TSharedPtr<FFlockPlaytestSelfTest> Self = WeakSelf.Pin())
			{
...
void FFlockPlaytestSelfTest::CheckMissingApiKeyRefused()
{
	TMap<FString, FString> SentHeaders = Headers();
...
void FFlockPlaytestSelfTest::CheckVersionWithNoPlaytestRefused()
{
	TMap<FString, FString> SentHeaders = Headers();
...
void FFlockPlaytestSelfTest::CheckSessionStarted()
{
	const TWeakObjectPtr<UFlockPlaytestSubsystem> WeakPlaytest = Playtest;
	WaitUntil([WeakPlaytest]()
		{
			...
			const EFlockPlaytestSessionState State = WeakPlaytest->GetPlaytestSessionState();
			return State != EFlockPlaytestSessionState::NotStarted && State != EFlockPlaytestSessionState::Starting;
		},
		Options.WaitSeconds, [this](bool)
		{
			const UFlockPlaytestSubsystem* PlaytestNow = Playtest.Get();
			...
			{
			case EFlockPlaytestSessionState::Started:
				bSessionStarted = true;
				Finish(EFlockPlaytestSelfTestOutcome::Passed, FString::Printf(TEXT("session %s, for the player's %s"),
					*PlaytestNow->GetPlaytestSessionId(),
					PlaytestNow->GetPlaytestIdentity().SteamId.IsEmpty() ? TEXT("device id") : TEXT("Steam id")));
				return;
			case EFlockPlaytestSessionState::NotStarted:
				Finish(EFlockPlaytestSelfTestOutcome::Failed, FString::Printf(TEXT("no session started within %.0f seconds. A playtest "
				...
				return;
			case EFlockPlaytestSessionState::Starting:
				Finish(EFlockPlaytestSelfTestOutcome::Failed, FString::Printf(TEXT("the session start was sent and not answered "
					"within %.0f seconds"), Options.WaitSeconds));
				return;
			case EFlockPlaytestSessionState::Ended:
				Finish(EFlockPlaytestSelfTestOutcome::Failed, TEXT("this launch's session had already ended, so there is none to "
					"check. A launch has one session: start the game again to run the self-test."));
				return;
			case EFlockPlaytestSessionState::NoPlayerIdentity:
				Finish(EFlockPlaytestSelfTestOutcome::Failed, TEXT("no session was started, because there was neither a Steam id "
					"nor a device id to send; the log above says why"));
				return;
			default:
				Finish(EFlockPlaytestSelfTestOutcome::Failed, TEXT("Protokite did not create this launch's session; the log above "
					"says why"));
				return;
			}
		});
...
void FFlockPlaytestSelfTest::CheckSessionWithNoPlayerRefused()
{
	...
	Client->StartPlaytestSession(ProtokiteApiUrl(), SentHeaders, Request,
		[WeakSelf, SentHeaders](TFlockResult<FFlockPlaytestSessionStartResult> Result)
		{
			const TSharedPtr<FFlockPlaytestSelfTest> Self = WeakSelf.Pin();
			if (!Self.IsValid())
			{
...
void FFlockPlaytestSelfTest::CheckClosedPlaytestRefused()
{
	if (Options.ClosedPlaytestGameVersionId.IsEmpty())
	{
		Finish(EFlockPlaytestSelfTestOutcome::Skipped, TEXT("no closed playtest was named. Pass the Game Version ID of a closed "
			"playtest of this game to check it: FlockPlaytest.SelfTest <Game Version ID>."));
		return;
	}
...
void FFlockPlaytestSelfTest::CheckExceptionReported()
{
	UFlockPlaytestSubsystem* PlaytestNow = Playtest.Get();
	UFlockSubsystem* FlockNow = Flock.Get();
	if (!PlaytestNow->IsPlaytestFeatureEnabled(FlockPlaytestFeatures::ExceptionCapturing))
	{
		Finish(EFlockPlaytestSelfTestOutcome::Skipped, TEXT("this playtest does not turn exception capturing on"));
		return;
	}
	if (!FlockNow->GetExceptionCaptureCoverage().bEnabled)
	{
		Finish(EFlockPlaytestSelfTestOutcome::Failed, TEXT("this playtest turns exception capturing on, but the Flock SDK is not "
	...
	{
		Finish(EFlockPlaytestSelfTestOutcome::Failed, TEXT("the Flock SDK is not collecting: analytics is off, or the player's "
	...
	{
		Finish(EFlockPlaytestSelfTestOutcome::Skipped, ReportsNotWatchableReason);
		return;
	}
	...
	{
		Finish(EFlockPlaytestSelfTestOutcome::Failed, TEXT("no Blueprint fault could be raised"));
		return;
	}
	...
	WaitUntil([PendingCount, CountBefore]() { return PendingCount() > CountBefore; }, SecondsForAFaultToBeQueued,
		[this, PendingCount, CountBefore](bool bQueued)
		{
			...
			if (!bQueued)
			{
				Finish(EFlockPlaytestSelfTestOutcome::Failed, TEXT("the same Blueprint fault was raised twice and nothing was queued"));
				return;
			}
			if (Queued != 1)
			{
				// The counter-case: a repeat queued as a report of its own means repeats are not being counted.
				Finish(EFlockPlaytestSelfTestOutcome::Failed, FString::Printf(TEXT("the same fault raised twice queued %d reports, "
...
void FFlockPlaytestSelfTest::CheckPlaytestEventRecorded()
{
	UFlockPlaytestSubsystem* PlaytestNow = Playtest.Get();
	UFlockSubsystem* FlockNow = Flock.Get();
	if (!PlaytestNow->IsPlaytestFeatureEnabled(FlockPlaytestFeatures::HeavyAnalytics))
	{
		Finish(EFlockPlaytestSelfTestOutcome::Skipped, TEXT("this playtest does not turn heavy analytics on"));
		return;
	}
	const FFlockAnalyticsProvider* Analytics = FlockNow->GetAnalyticsProvider();
	if (Analytics == nullptr)
	{
		Finish(EFlockPlaytestSelfTestOutcome::Failed, TEXT("this playtest turns heavy analytics on, but the Flock SDK's analytics "
	...
	Properties.Set(TEXT("sent_by"), TEXT("FlockPlaytest.SelfTest"));
	...
	const bool bOwnNameAccepted = PlaytestNow->RecordPlaytestEvent(FlockPlaytestEvents::PerformanceWindow);

	if (!bRecorded)
	{
		Finish(EFlockPlaytestSelfTestOutcome::Failed, FString::Printf(TEXT("%s was refused; the log above says why"), SelfTestEventName));
		return;
	}
	if (bOwnNameAccepted)
	{
		Finish(EFlockPlaytestSelfTestOutcome::Failed, FString::Printf(TEXT("the game was allowed to record %s, which is the "
			"plugin's own event"), FlockPlaytestEvents::PerformanceWindow));
		return;
	}
	if (!FlockKeepsReportsUntilSent())
	{
		Finish(EFlockPlaytestSelfTestOutcome::Skipped, FString::Printf(TEXT("%s was recorded and %s refused, but %s"),
			SelfTestEventName, FlockPlaytestEvents::PerformanceWindow, ReportsNotWatchableReason));
		return;
	}
	if (CountAfterRecording != CountBefore + 1)
	{
		Finish(EFlockPlaytestSelfTestOutcome::Failed, FString::Printf(TEXT("%s was accepted, and %d events were queued for it"),
	...
	FlushAndWaitUntilSent([WeakFlock]() -> int32
		...
		CountAfterRecording, FString::Printf(TEXT("%s was queued, and %s refused"), SelfTestEventName, FlockPlaytestEvents::PerformanceWindow));
}

void FFlockPlaytestSelfTest::FlushAndWaitUntilSent(TFunction<int32()> PendingCount, int32 CountBeforeTheFlush, const FString& WhatWasQueued)
{
	FFlockAnalyticsProvider* Analytics = Flock.IsValid() ? Flock->GetAnalyticsProvider() : nullptr;
	if (Analytics == nullptr)
	{
		Finish(EFlockPlaytestSelfTestOutcome::Failed, TEXT("the Flock SDK's analytics went away before the flush"));
		return;
	}
	...
	Analytics->Flush([WeakSelf, PendingCount, CountBeforeTheFlush, WhatWasQueued](TFlockResult<FFlockAnalyticsAck> Result)
		{
			...
			if (!Result.bSuccess)
			{
				Self->Finish(EFlockPlaytestSelfTestOutcome::Failed, FString::Printf(TEXT("%s, and the flush failed: %s"),
			...
			Self->WaitUntil([PendingCount, CountBeforeTheFlush]() { return PendingCount() < CountBeforeTheFlush; },
				...
				{
					RawSelf->Finish(bSent ? EFlockPlaytestSelfTestOutcome::Passed : EFlockPlaytestSelfTestOutcome::Failed,
...
void FFlockPlaytestSelfTest::SendFormExpectingRefusal(const FFlockPlaytestFormSubmission& Submission, int32 ExpectedStatus,
	const FString& QuestionItMustName)
{
	const TWeakPtr<FFlockPlaytestSelfTest> WeakSelf = AsShared();
	Client->SubmitFeedbackForm(Headers(), Submission,
		[WeakSelf, ExpectedStatus, QuestionItMustName](TFlockResult<FFlockPlaytestFormSubmitResult> Result)
		{
			if (const TSharedPtr<FFlockPlaytestSelfTest> Self = WeakSelf.Pin())
			{
...
void FFlockPlaytestSelfTest::CheckFormMissingANeededAnswerRefused()
{
	const UFlockPlaytestSubsystem* PlaytestNow = Playtest.Get();
	if (!PlaytestNow->CanOpenFeedbackForm())
	{
		Finish(EFlockPlaytestSelfTestOutcome::Skipped, TEXT("this playtest publishes no feedback form"));
		return;
	}
	const FFlockPlaytestForm& Form = PlaytestNow->GetPlaytestConfig().Form;
	const FFlockPlaytestFormField* Needed = Form.Fields.FindByPredicate([](const FFlockPlaytestFormField& Field) { return Field.Required; });
	if (Needed == nullptr)
	{
		Finish(EFlockPlaytestSelfTestOutcome::Skipped, TEXT("no question on this playtest's form needs an answer"));
		return;
	}

	// Every other question answered, so the only thing the server can object to is the one left out.
	FFlockPlaytestForm FormWithoutIt = Form;
	const FString NeededId = Needed->Id;
	FormWithoutIt.Fields.RemoveAll([&NeededId](const FFlockPlaytestFormField& Field) { return Field.Id.Equals(NeededId, ESearchCase::CaseSensitive); });
	const FFlockPlaytestFormAnswers Answers = FlockPlaytestAnswerEveryQuestion(FormWithoutIt, SelfTestAnswerText);
	SendFormExpectingRefusal(MakeFormSubmission(PlaytestNow->GetPlaytestSessionId(), SerializeSelfTestAnswers(Answers, Form)),
		422, NeededId);
}

void FFlockPlaytestSelfTest::CheckFormWithAnOptionNotOnTheListRefused()
{
	const UFlockPlaytestSubsystem* PlaytestNow = Playtest.Get();
	if (!PlaytestNow->CanOpenFeedbackForm())
	{
	...
	const FFlockPlaytestForm& Form = PlaytestNow->GetPlaytestConfig().Form;
	const FFlockPlaytestFormField* Select = Form.Fields.FindByPredicate([](const FFlockPlaytestFormField& Field)
		{
			return Field.IsOfKind(FlockPlaytestFormFieldTypes::Select) && Field.Options.Num() > 0;
		});
	if (Select == nullptr)
	{
		Finish(EFlockPlaytestSelfTestOutcome::Skipped, TEXT("this playtest's form has no question with a list of options"));
		return;
	}

	FFlockPlaytestFormAnswers Answers = FlockPlaytestAnswerEveryQuestion(Form, SelfTestAnswerText);
	Answers.SetChosenOption(Select->Id, MakeOptionNotOnTheList(Select->Options));
	SendFormExpectingRefusal(MakeFormSubmission(PlaytestNow->GetPlaytestSessionId(), SerializeSelfTestAnswers(Answers, Form)),
		422, Select->Id);
}

void FFlockPlaytestSelfTest::CheckFormForASessionThatDoesNotExistRefused()
{
	const UFlockPlaytestSubsystem* PlaytestNow = Playtest.Get();
	if (!PlaytestNow->CanOpenFeedbackForm())
	{
	...
	SendFormExpectingRefusal(MakeFormSubmission(SessionIdThatDoesNotExist,
		SerializeSelfTestAnswers(FlockPlaytestAnswerEveryQuestion(Form, SelfTestAnswerText), Form)), 404, FString());
}

void FFlockPlaytestSelfTest::CheckFormTaken()
{
	const UFlockPlaytestSubsystem* PlaytestNow = Playtest.Get();
	if (!PlaytestNow->CanOpenFeedbackForm())
	{
	...
	const FFlockPlaytestForm& Form = PlaytestNow->GetPlaytestConfig().Form;
	const FFlockPlaytestFormAnswers Answers = FlockPlaytestAnswerEveryQuestion(Form, SelfTestAnswerText);
	const TArray<FFlockPlaytestFormProblem> Problems = Answers.FindProblems(Form);
	if (Problems.Num() > 0)
	{
		Finish(EFlockPlaytestSelfTestOutcome::Failed, FString::Printf(TEXT("the self-test's own answers would be refused: question "
	...
	Client->SubmitFeedbackForm(Headers(), MakeFormSubmission(SessionId, SerializeSelfTestAnswers(Answers, Form)),
		[WeakSelf, SessionId, QuestionCount = Form.Fields.Num()](TFlockResult<FFlockPlaytestFormSubmitResult> Result)
		{
			const TSharedPtr<FFlockPlaytestSelfTest> Self = WeakSelf.Pin();
			if (!Self.IsValid())
			{
			...
			if (Result.bSuccess)
			{
				Self->Finish(EFlockPlaytestSelfTestOutcome::Passed, FString::Printf(TEXT("%d answers taken for session %s"),
					QuestionCount, *SessionId));
			}
			else
			{
				Self->Finish(EFlockPlaytestSelfTestOutcome::Failed, Result.Error.ToDisplayText());
			}
		});
}

void FFlockPlaytestSelfTest::CheckUploadLinkForASessionThatDoesNotExistRefused()
{
	const TWeakPtr<FFlockPlaytestSelfTest> WeakSelf = AsShared();
	Client->RequestRecordingUploadLink(ProtokiteApiUrl(), Headers(), SessionIdThatDoesNotExist, FlockPlaytestRecordingContentTypes::WebM,
		[WeakSelf](TFlockResult<FFlockPlaytestRecordingUploadLink> Result)
		{
			if (const TSharedPtr<FFlockPlaytestSelfTest> Self = WeakSelf.Pin())
			{
				Self->FinishWithRefusal(Result.bSuccess, Result.Error, 404);
...
void FFlockPlaytestSelfTest::CheckRecordingUploaded()
{
	UFlockPlaytestSubsystem* PlaytestNow = Playtest.Get();
	...
	if (!PlaytestNow->CanSendTheRecording())
	{
		...
		{
			Finish(EFlockPlaytestSelfTestOutcome::Skipped, TEXT("this launch cannot render (-nullrhi), so nothing is recorded. Run "
				"it with a window to check the recording."));
		}
		else
		{
			Finish(EFlockPlaytestSelfTestOutcome::Failed, TEXT("this playtest records video, but this launch has no recording it "
				"can send; the log above says why"));
		}
		return;
	}

	Listener = NewObject<UFlockPlaytestSelfTestListener>();
	Listener->AddToRoot();
	...
	PlaytestNow->OnRecordingUploadFinished.AddDynamic(Listener, &UFlockPlaytestSelfTestListener::HandleRecordingUploadFinished);

	if (!PlaytestNow->StopVideoRecordingAndUploadIt())
	{
		StopListeningForTheUpload();
		Finish(EFlockPlaytestSelfTestOutcome::Failed, TEXT("the recording could not be stopped to be sent"));
		return;
	}
	...
	WaitUntil([this]() { return bUploadFinished; }, Options.UploadWaitSeconds, [this](bool bHeard)
		{
			...
			if (!bHeard)
			{
				Finish(EFlockPlaytestSelfTestOutcome::Failed, FString::Printf(TEXT("the upload had not finished after %.0f seconds"),
					Options.UploadWaitSeconds));
				return;
			}
			Finish(bUploaded ? EFlockPlaytestSelfTestOutcome::Passed : EFlockPlaytestSelfTestOutcome::Failed,
				bUploaded ? FString(TEXT("uploaded to this launch's session, and no longer kept on disk")) : UploadWhyNot);
		});
}

void FFlockPlaytestSelfTest::CheckEndForASessionThatDoesNotExistRefused()
{
	const TWeakPtr<FFlockPlaytestSelfTest> WeakSelf = AsShared();
	Client->EndPlaytestSession(ProtokiteApiUrl(), Headers(), SessionIdThatDoesNotExist,
		[WeakSelf](TFlockResult<FFlockPlaytestSessionEndResult> Result)
		{
			if (const TSharedPtr<FFlockPlaytestSelfTest> Self = WeakSelf.Pin())
			{
				Self->FinishWithRefusal(Result.bSuccess, Result.Error, 404);
...
void FFlockPlaytestSelfTest::CheckSessionEnded()
{
	UFlockPlaytestSubsystem* PlaytestNow = Playtest.Get();
	const FString SessionId = PlaytestNow->GetPlaytestSessionId();
	const TWeakPtr<FFlockPlaytestSelfTest> WeakSelf = AsShared();
	...
	const bool bSent = PlaytestNow->EndPlaytestSession([WeakSelf, WeakPlaytest, SessionId](bool bEnded, const FString& WhyNot)
		{
			...
			const bool bStateEnded = WeakPlaytest.IsValid() && WeakPlaytest->GetPlaytestSessionState() == EFlockPlaytestSessionState::Ended;
			if (bEnded && bStateEnded)
			{
				Self->Finish(EFlockPlaytestSelfTestOutcome::Passed, FString::Printf(TEXT("session %s ended"), *SessionId));
			}
			else
			{
				Self->Finish(EFlockPlaytestSelfTestOutcome::Failed, bEnded
	...
	if (!bSent)
	{
		Finish(EFlockPlaytestSelfTestOutcome::Failed, TEXT("no end was sent: the playtest has no session to end"));
	}
}

void FFlockPlaytestSelfTest::EndSessionThatShouldNotExist(const TMap<FString, FString>& SessionHeaders, const FString& PlaytestSessionId)
{
	Client->EndPlaytestSession(ProtokiteApiUrl(), SessionHeaders, PlaytestSessionId,
		[PlaytestSessionId](TFlockResult<FFlockPlaytestSessionEndResult> Result)
		{
			if (Result.bSuccess)
			{
				UE_LOG(LogFlockPlaytest, Display, TEXT("Playtest self-test: ended session %s, which Protokite should have refused to create."),
					*PlaytestSessionId);
			}
			else
			{
				UE_LOG(LogFlockPlaytest, Warning, TEXT("Playtest self-test: session %s, which Protokite should have refused to create, "
...
namespace
{
	...
	FAutoConsoleCommandWithWorldArgsAndOutputDevice PlaytestSelfTestCommand(
		...
		FConsoleCommandWithWorldArgsAndOutputDeviceDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World, FOutputDevice& Output)
		{
			...
			if (Args.Num() > 1)
			{
				Output.Log(TEXT("Usage: FlockPlaytest.SelfTest [Game Version ID of a closed playtest of this game]."));
				return;
			}
			...
			{
				Output.Log(TEXT("No game instance with the Flock SDK and the Flock Playtest plugin runs in this world."));
				return;
			}
			...
			GRunningPlaytestSelfTest = FFlockPlaytestSelfTest::Start(Playtest, Flock, Options);
		}));
}
