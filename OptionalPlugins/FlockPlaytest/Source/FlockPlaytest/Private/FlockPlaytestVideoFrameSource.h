...
{
public:
	virtual ~IFlockPlaytestVideoFrameSource() = default;
	...
	virtual void TakeCapturedFrames(TArray<FFlockPlaytestVideoFrame>& OutFrames) = 0;

	/** Stops capturing for good, and hands over the frames that had already arrived. Frames still on their way are dropped. */
	virtual void Stop(TArray<FFlockPlaytestVideoFrame>& OutFrames) = 0;
};
 ...
 */
class FFlockPlaytestGameViewportFrameSource : public IFlockPlaytestVideoFrameSource
{
public:
	/**
	 ...
	 */
	static TSharedPtr<IFlockPlaytestVideoFrameSource> Create(const UGameInstance* GameInstance, FIntPoint MaxVideoSize, FString& OutWhyNot);

	virtual ~FFlockPlaytestGameViewportFrameSource() override;
...
private:
	FFlockPlaytestGameViewportFrameSource(const TSharedRef<FSceneViewport>& InViewport, FIntPoint InFrameSize);
	...
	void StopGrabber(TArray<FFlockPlaytestVideoFrame>& OutFrames);
