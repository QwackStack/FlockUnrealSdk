...
FFlockPlaytestPerformanceTimeline::FFlockPlaytestPerformanceTimeline(TFunction<double()> InReadHitchThresholdMs,
...
bool FFlockPlaytestPerformanceTimeline::AddFrame(double FrameSeconds, uint64 EngineFrameNumber, FFlockPlaytestPerformanceWindow& OutWindow)
{
	if (FrameSeconds <= 0.0)
	{
		return false;
	...
	FrameTimesMs.Sort();
	OutWindow = FFlockPlaytestPerformanceWindow();
...
void FFlockPlaytestPerformanceTimeline::Reset()
{
	FrameTimesMs.Reset();
	ElapsedSeconds = 0.0;
	bLeaveOutNextFrame = false;
...
double FFlockPlaytestPerformanceTimeline::Percentile(const TArray<double>& SortedFrameTimesMs, double Percent)
{
