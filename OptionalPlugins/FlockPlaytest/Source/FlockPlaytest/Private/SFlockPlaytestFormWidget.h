 ...
 */
class SFlockPlaytestFormWidget : public SCompoundWidget
{
public:
	DECLARE_DELEGATE_OneParam(FOnFormSubmitted, const FFlockPlaytestFormAnswers&);
	DECLARE_DELEGATE_RetVal(bool, FCanSendRecording);
	DECLARE_DELEGATE_RetVal(bool, FOnSendRecording);

	SLATE_BEGIN_ARGS(SFlockPlaytestFormWidget) {}
		/** The published form to draw. */
		SLATE_ARGUMENT(FFlockPlaytestForm, Form)
	...
	const FFlockPlaytestFormAnswers& GetAnswers() const { return Answers; }

	/** Checks the answers and, when there is nothing wrong, hands them over. Returns what was wrong, if anything. */
	TArray<FFlockPlaytestFormProblem> TrySubmitForTesting() { return TrySubmit(); }
...
private:
	...
	TSharedRef<SWidget> BuildField(const FFlockPlaytestFormField& Field);
	TSharedRef<SWidget> BuildAnswerControl(const FFlockPlaytestFormField& Field);
	TSharedRef<SWidget> BuildRating(const FFlockPlaytestFormField& Field);
	TSharedRef<SWidget> BuildSelect(const FFlockPlaytestFormField& Field);
