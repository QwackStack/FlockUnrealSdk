 ...
 * its upload.
 */
class FFlockPlaytestFakeFileUploader : public IFlockFileUploader
{
public:
	/** Route a URL fragment to a successful upload. Last route for a fragment wins. */
	FFlockPlaytestFakeFileUploader& On(const FString& UrlFragment)
	{
	...
	FFlockPlaytestFakeFileUploader& OnStatus(const FString& UrlFragment, int32 StatusCode, const FString& Detail = FString())
	{
		FRoute& Route = Replace(UrlFragment);
		Route.Outcome.Result = EFlockHttpResult::Success;
	...
	FFlockPlaytestFakeFileUploader& OnTransportFailure(const FString& UrlFragment, EFlockHttpResult Result)
	{
		FRoute& Route = Replace(UrlFragment);
	 ...
	 */
	FFlockPlaytestFakeFileUploader& SignedFor(const FString& UrlFragment, const FString& ContentType)
	{
