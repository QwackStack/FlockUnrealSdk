...
bool FFlockPlaytestVideoSubsystemTestRecordingStopsAfterItsSecondsTest::RunTest(const FString& Parameters)
{
	...
	TestEqual(TEXT("In a run's folder under TestVideos"), FPaths::GetCleanFilename(FPaths::GetPath(FPaths::GetPath(Path))),
		FString(FFlockPlaytestRecordingsFolder::TestVideoFolderName));
	FVideoFileRead File;
	if (TestTrue(TEXT("The file reads back"), ReadVideoFile(Path, File)))
	{
...
namespace
{
	...
	FFlockPlaytestRecordingSession ReadSavedRecordingSession(const FString& RunFolder, FString& OutText)
	{
		FFlockPlaytestRecordingSession Session;
		const FString Path = FPaths::Combine(RunFolder, FFlockPlaytestRecordingsFolder::SessionFileName);
	...
	int32 CountRecordingRunFolders(const FString& Recordings, EFlockPlaytestRecordingKind Kind)
	{
		TArray<FString> Names;
		IFileManager::Get().FindFiles(Names, *FPaths::Combine(FFlockPlaytestRecordingsFolder::GetKindFolder(Recordings, Kind), TEXT("*")),
			/*Files*/ false, /*Directories*/ true);
		return Names.Num();
	}
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FFlockPlaytestVideoRecordingsSavesTheSessionTest,
	"Flock.Playtest.Video.Recordings.SavesTheSessionBesideThePlaytestRecording",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FFlockPlaytestVideoRecordingsSavesTheSessionTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedVideoSettings Video;
	{
		...
		Fixture.RegisterFlockSession(FirstFlockSessionId);
		TestEqual(TEXT("Precondition: the Protokite session started"), Fixture.SessionStarts(), 1);
		const FFlockPlaytestRecordingSession Saved = ReadSavedRecordingSession(RunFolder, Text);
	...
	}
	{
		...
		TestEqual(TEXT("Precondition: the Protokite session started"), Fixture.SessionStarts(), 1);
		Fixture.Flock->ShutdownSdk();
		Fixture.AnswerConfig(FFlockPlaytestFakeTransport::Status(200, ConfigBody(VideoRecordingsSecondGameVersionId, false, true)));
		Fixture.Flock->InitializeWithConfig(MakeFlockConfig(VideoRecordingsSecondGameVersionId));
		ExpectPlaytestStatus(*this, TEXT("Ready with the other version"), Fixture.Playtest->GetStatus(), EFlockPlaytestStatus::Ready);
		Log->bViewportExists = true;
		Fixture.Playtest->TickVideoRecordingForTesting(1.f / 60.f);
		TestTrue(TEXT("Precondition: recording"), Fixture.Playtest->IsRecordingVideo());
		const TArray<FString> Files = RecordingFilesIn(RecordingsFolder(Fixture));
		...
		{
			FString Text;
			const FFlockPlaytestRecordingSession Saved = ReadSavedRecordingSession(FPaths::GetPath(Files[0]), Text);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FFlockPlaytestVideoRecordingsKeptForTheNextLaunchTest,
	"Flock.Playtest.Video.Recordings.KeptForALaterLaunchToUpload",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FFlockPlaytestVideoRecordingsKeptForTheNextLaunchTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedVideoSettings Video;
	...
	FPlaytestLogCapture Capture;
	Next.AnswerConfig(FFlockPlaytestFakeTransport::Status(200, ConfigBody(VideoRecordingsSecondGameVersionId, false, true)));
	...
	const TArray<FFlockPlaytestRecordingWaitingToUpload> Waiting = FFlockPlaytestRecordingsFolder::FindRecordingsWaitingToUpload(RecordingsFolder(Earlier));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FFlockPlaytestVideoRecordingsKeepsWhatItsLaunchOwnsTest,
	"Flock.Playtest.Video.Recordings.NothingTouchesARecordingItsLaunchStillOwns",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FFlockPlaytestVideoRecordingsKeepsWhatItsLaunchOwnsTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedVideoSettings Video;
	...
	Owner.AnswerConfig(FFlockPlaytestFakeTransport::Status(200, ConfigBody(VideoRecordingsSecondGameVersionId, false, true)));
	Owner.Flock->InitializeWithConfig(MakeFlockConfig(VideoRecordingsSecondGameVersionId));
	ExpectPlaytestStatus(*this, TEXT("Ready with the other version"), Owner.Playtest->GetStatus(), EFlockPlaytestStatus::Ready);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FFlockPlaytestVideoRecordingsKeepsTestVideosTest,
	"Flock.Playtest.Video.Recordings.ATestVideoIsKeptByLaterLaunches",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FFlockPlaytestVideoRecordingsKeepsTestVideosTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(false, UsableUrl);
	FScopedVideoSettings Video;
	FPlaytestFixture Earlier;
	...
	TestEqual(TEXT("And is never listed for upload"), FFlockPlaytestRecordingsFolder::FindRecordingsWaitingToUpload(RecordingsFolder(Earlier)).Num(), 0);
	return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FFlockPlaytestVideoRecordingsCutToFitTheBudgetTest,
	"Flock.Playtest.Video.Recordings.ARecordingIsCutToFitTheDiskBudget",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FFlockPlaytestVideoRecordingsCutToFitTheBudgetTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	...
	FString OldTestVideo;
	{
		...
		FString Error;
		const TSharedPtr<FFlockPlaytestRecordingRun> Old = FFlockPlaytestRecordingRun::CreateNamedForTesting(RecordingsFolder(Fixture),
			EFlockPlaytestRecordingKind::TestVideo, TEXT("20200101-000000-00000000"), /*ReservedBytes*/ 0, Error);
	...
	FString OlderPlaytestRecording;
	{
		...
		FString Error;
		const TSharedPtr<FFlockPlaytestRecordingRun> Older = FFlockPlaytestRecordingRun::CreateNamedForTesting(RecordingsFolder(Fixture),
			EFlockPlaytestRecordingKind::Playtest, TEXT("20190101-000000-00000000"), /*ReservedBytes*/ 0, Error);
	...
	{
		FString Reserved;
		FFileHelper::LoadFileToString(Reserved, *FPaths::Combine(FPaths::GetPath(Recording[0]), FFlockPlaytestRecordingsFolder::ReservedBytesFileName));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FFlockPlaytestVideoRecordingsNoRoomRecordsNothingTest,
	"Flock.Playtest.Video.Recordings.NoRoomInTheDiskBudgetRecordsNothing",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FFlockPlaytestVideoRecordingsNoRoomRecordsNothingTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	...
	FString Error;
	const TSharedPtr<FFlockPlaytestRecordingRun> StillRunning = FFlockPlaytestRecordingRun::Create(RecordingsFolder(Fixture),
		EFlockPlaytestRecordingKind::TestVideo, /*ReservedBytes*/ 1024 * 1024, Error);
	...
	TestEqual(TEXT("No run folder is made for it"), CountRecordingRunFolders(RecordingsFolder(Fixture), EFlockPlaytestRecordingKind::Playtest), 0);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FFlockPlaytestVideoRecordingKeepsWhatWasWrittenWhenAWriteFailsTest,
	"Flock.Playtest.Video.Recording.KeepsWhatWasWrittenWhenAWriteFails",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::ClientContext | EAutomationTestFlags::EngineFilter)
...
bool FFlockPlaytestVideoRecordingKeepsWhatWasWrittenWhenAWriteFailsTest::RunTest(const FString& Parameters)
{
	...
	FString Error;
	{
		...
		const TSharedPtr<FFlockPlaytestVideoRecording> Recording = FFlockPlaytestVideoRecording::Start(Source, FFlockPlaytestVideoSettings(), Path, Error,
		...
		TOptional<EFlockPlaytestVideoStopReason> StopReason;
		...
		TestTrue(TEXT("The recording asks to stop because its file could not be written"),
			StopReason.IsSet() && StopReason.GetValue() == EFlockPlaytestVideoStopReason::CouldNotWrite);
		Recording->StopCapturing(StopReason.Get(EFlockPlaytestVideoStopReason::StoppedByGame));
		Recording->WaitUntilWritten();
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FFlockPlaytestVideoRecordingsLaunchDoesNotWaitForTheFolderTest,
	"Flock.Playtest.Video.Recordings.GoingThroughEarlierRecordingsDoesNotHoldUpTheLaunch",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FFlockPlaytestVideoRecordingsLaunchDoesNotWaitForTheFolderTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(false, UsableUrl);
	FPlaytestFixture Fixture;
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FFlockPlaytestVideoRecordingsTestVideoDeletesNoWaitingUploadTest,
	"Flock.Playtest.Video.Recordings.AShortTestVideoDeletesNoRecordingWaitingToUpload",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FFlockPlaytestVideoRecordingsTestVideoDeletesNoWaitingUploadTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(false, UsableUrl);
	...
	FString WaitingRecording;
	{
		...
		FString Error;
		const TSharedPtr<FFlockPlaytestRecordingRun> Earlier = FFlockPlaytestRecordingRun::CreateNamedForTesting(RecordingsFolder(Fixture),
			EFlockPlaytestRecordingKind::Playtest, TEXT("20190101-000000-00000000"), /*ReservedBytes*/ 0, Error);
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FFlockPlaytestVideoRecordingsKeepsWhatWasWrittenWhenAWriteFailsTest,
	"Flock.Playtest.Video.Recordings.TheLaunchKeepsWhatWasWrittenWhenAWriteFails",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FFlockPlaytestVideoRecordingsKeepsWhatWasWrittenWhenAWriteFailsTest::RunTest(const FString& Parameters)
{
	FScopedPlaytestSettings Settings(true, UsableUrl);
	FScopedVideoSettings Video;
	FPlaytestFixture Fixture;
