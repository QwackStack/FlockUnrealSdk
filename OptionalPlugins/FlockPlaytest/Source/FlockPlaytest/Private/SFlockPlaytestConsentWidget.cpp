...
namespace
{
	 ...
	 * apart.
	 */
	constexpr FFlockPlaytestConsentOption Options[] = {
		{
...
TArrayView<const FFlockPlaytestConsentOption> SFlockPlaytestConsentWidget::GetOptions()
{
	return MakeArrayView(Options);
}

void SFlockPlaytestConsentWidget::Construct(const FArguments& InArgs)
{
	OnChosen = InArgs._OnChosen;

	const TSharedRef<SVerticalBox> Answers = SNew(SVerticalBox);
	for (const FFlockPlaytestConsentOption& Option : Options)
	{
...
TSharedRef<SWidget> SFlockPlaytestConsentWidget::BuildOption(const FFlockPlaytestConsentOption& Option)
{
	const EFlockPlaytestConsentChoice Choice = Option.Choice;
	return SNew(SButton)
		.HAlign(HAlign_Fill)
		.ContentPadding(FMargin(14.f, 12.f))
		.OnClicked(FOnClicked::CreateSP(this, &SFlockPlaytestConsentWidget::Choose, Choice))
		[
			SNew(SVerticalBox)
...
FReply SFlockPlaytestConsentWidget::Choose(EFlockPlaytestConsentChoice Choice)
{
