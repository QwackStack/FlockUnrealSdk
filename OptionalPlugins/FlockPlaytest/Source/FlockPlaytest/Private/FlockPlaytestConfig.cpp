...
namespace
{
	...
	void ReadFields(const TSharedRef<FJsonObject>& FormObject, TArray<FFlockPlaytestFormField>& OutFields)
	{
		const TArray<TSharedPtr<FJsonValue>>* Values = nullptr;
		...
		for (const TSharedPtr<FJsonValue>& Value : *Values)
		{
			...
			FFlockPlaytestFormField Question;
...
bool FFlockPlaytestConfig::IsFeatureEnabled(const FString& FeatureName) const
{
	const bool* Enabled = Features.Find(FeatureName);
	return Enabled != nullptr && *Enabled;
}

bool FFlockPlaytestConfig::FromWireObject(const TSharedRef<FJsonObject>& Object, FFlockPlaytestConfig& OutConfig,
	FString& OutError)
{
	OutConfig = FFlockPlaytestConfig();
