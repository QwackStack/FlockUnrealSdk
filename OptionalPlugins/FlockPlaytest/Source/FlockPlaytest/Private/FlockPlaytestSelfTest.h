...
#include "FlockPlaytestSelfTest.generated.h"

class FFlockProtokiteClient;
class IFlockHttpAdapter;
class UFlockPlaytestSubsystem;
class UFlockSubsystem;
struct FFlockError;
struct FFlockPlaytestFormSubmission;

/** How one self-test step came out. */
enum class EFlockPlaytestSelfTestOutcome : uint8
{
	Passed,
	Failed,
...
struct FFlockPlaytestSelfTestStep
{
	FString Name;
	EFlockPlaytestSelfTestOutcome Outcome = EFlockPlaytestSelfTestOutcome::Failed;
	FString Detail;
};

/** The self-test's steps, in the order they run. Each counter-case sits beside the step it proves. */
namespace FlockPlaytestSelfTestSteps
{
...
UCLASS()
class UFlockPlaytestSelfTestListener : public UObject
{
	GENERATED_BODY()
 ...
 * its counter-case, ending on a count of what passed. Development builds only, started by FlockPlaytest.SelfTest.
 *
 ...
 */
class FFlockPlaytestSelfTest : public TSharedFromThis<FFlockPlaytestSelfTest>
{
public:
	struct FOptions
	{
	...
	static TSharedRef<FFlockPlaytestSelfTest> Start(UFlockPlaytestSubsystem* Playtest, UFlockSubsystem* Flock,
		const FOptions& Options, TFunction<void(const TArray<FFlockPlaytestSelfTestStep>&)> OnFinished = nullptr);

	~FFlockPlaytestSelfTest();

	bool IsFinished() const { return bFinished; }
	const TArray<FFlockPlaytestSelfTestStep>& GetSteps() const { return Steps; }
...
private:
	...
	struct FStepToRun
	{
		const TCHAR* Name;
		ENeeds Needs;
		void (FFlockPlaytestSelfTest::*Run)();
	};

	FFlockPlaytestSelfTest() = default;

	void RunNextStep();

	/** Records the running step's outcome, logs it and starts the next. Called exactly once per step. */
	void Finish(EFlockPlaytestSelfTestOutcome Outcome, const FString& Detail);
	...
	FFlockPlaytestFormSubmission MakeFormSubmission(const FString& PlaytestSessionId, const FString& AnswersJson) const;

	/** Sends a form expected to be refused with ExpectedStatus, naming QuestionItMustName when that is given. */
	void SendFormExpectingRefusal(const FFlockPlaytestFormSubmission& Submission, int32 ExpectedStatus, const FString& QuestionItMustName);
	...
	TFunction<void(const TArray<FFlockPlaytestSelfTestStep>&)> OnFinished;

	TArray<FStepToRun> StepsToRun;
	int32 NextStep = 0;
	const TCHAR* RunningStepName = nullptr;
	TArray<FFlockPlaytestSelfTestStep> Steps;
	...
	UFlockPlaytestSelfTestListener* Listener = nullptr;
